-- Fix for Customer Account Statement Function Error
-- This script specifically addresses the "cannot change return type" error

-- Use the exact syntax from the error message
...
BEGIN
  ...
  WITH combined_transactions AS (
    ...
    SELECT 
      ...
      COALESCE(p.description, 'Payment No: ' || p.payment_number) as trans_description,
    ...
    SELECT 
      ...
      COALESCE(
        jel.description, 
        je.description, 
        'Journal Entry No: ' || je.entry_number
      ) as trans_description,
      je.entry_number as ref_number,
...
'Professional customer account statement function with opening balance calculation, running balance tracking, and comprehensive transaction history from invoices, payments, and journal entries. Supports date range filtering and maintains proper accounting precision.';