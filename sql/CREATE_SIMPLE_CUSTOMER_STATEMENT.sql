-- ================================================================
-- SIMPLIFIED CUSTOMER ACCOUNT STATEMENT FUNCTION
-- ================================================================
-- This is a SIMPLIFIED version that works with minimal dependencies
-- Use this if the full version doesn't work
-- ================================================================
...
BEGIN
  ...
  WITH all_transactions AS (
    ...
    SELECT 
      ...
      COALESCE(i.notes, 'Invoice #' || i.invoice_number) as trans_desc,
    ...
    SELECT 
      ...
      COALESCE(p.notes, 'Payment #' || p.payment_number) as trans_desc,
...
DO $$
BEGIN
  RAISE NOTICE '✅ SIMPLIFIED function created successfully!';
  RAISE NOTICE '📋 This version works without journal entries or customer_accounts table';
  RAISE NOTICE '🔧 Test: SELECT * FROM get_customer_account_statement_by_code(company_id, customer_code, null, null);';
END $$;
