...
وبالرغم من المطالبات الودية المتكررة، إلا أن المدعى عليه لم يقم بسداد المبالغ المستحقة والتي بلغت ${overdueAmount.toLocaleString('ar-QA')} ريال قطري.`;
  }
  ...
  generateClaimsText(totalAmount: number): string {
    return `1. إلزام المدعى عليه بأن يؤدي للمدعية مبلغ (${totalAmount.toLocaleString('ar-QA')}) ريال قطري قيمة الإيجارات المتأخرة.
