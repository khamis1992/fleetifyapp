      ...
      <body>
        <div class="statement-container">
          ...
          <div class="warning-text">
            نحيطكم علماً بأن الكشف أدناه يوضح المستحقات المالية المترتبة عليكم، ونأمل منكم التكرم بسدادها في أقرب وقت ممكن.
          </div>
          ...
          <table>
              ...
              ${outstandingInvoices.map((invoice, index) => {
                ...
                return `
                  <tr>
                    ...
                    <td class="amount-cell">${outstanding.toLocaleString()} ر.ق</td>
            ...
            <tfoot>
              <tr style="background: #1e3a5f; color: white;">
                <td colspan="6" style="text-align: left; font-weight: bold; border-color: #1e3a5f;">إجمالي الفواتير المستحقة</td>
                <td colspan="2" style="font-weight: bold; font-size: 14px; border-color: #1e3a5f;">${totalOutstanding.toLocaleString()} ر.ق</td>
              </tr>
            </tfoot>
          </table>
          ` : ''}
          ...
          <table>
            ...
            <tbody>
              ${unpaidViolations.map((violation, index) => `
                <tr>
                  <td style="text-align: center;">${index + 1}</td>
                  <td>${violation.violation_number || violation.ticket_number || '-'}</td>
                  <td>${violation.violation_date ? format(new Date(violation.violation_date), 'dd/MM/yyyy') : '-'}</td>
                  <td>${violation.violation_type || violation.description || '-'}</td>
                  <td>${violation.plate_number || '-'}</td>
                  <td class="amount-cell">${(violation.fine_amount || violation.amount || 0).toLocaleString()} ر.ق</td>
                </tr>
              `).join('')}
            </tbody>
            <tfoot>
              <tr style="background: #1e3a5f; color: white;">
                <td colspan="5" style="text-align: left; font-weight: bold; border-color: #1e3a5f;">إجمالي المخالفات المرورية</td>
                <td style="font-weight: bold; font-size: 14px; border-color: #1e3a5f;">${totalViolationsAmount.toLocaleString()} ر.ق</td>
              </tr>
            </tfoot>
          </table>
          ` : ''}
          ...
          <div class="grand-total">
            ...
            <div class="amount">${totalDue.toLocaleString()} ر.ق</div>
          </div>
