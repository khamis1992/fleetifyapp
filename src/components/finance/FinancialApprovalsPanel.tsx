...
const priorityTone: Record<string, string> = {
  urgent: "border-0 bg-[#fdf1eb] text-[#b3694c]",
  high: "border-0 bg-[#FFF7ED] text-[#EA580C]",
  medium: "border-0 bg-[#e9f1f3] text-[#0284C7]",
  low: "border-0 bg-[#f7f8f4] text-[#5b6b52]",
};
        ...
        <CardHeader className="border-b border-slate-100 pb-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#fdf1eb] text-[#b3694c]">
                <FileCheck2 className="h-5 w-5" />
              </div>
              <div>
                <CardTitle className="text-lg font-black text-[#2c4136]">الموافقات المالية</CardTitle>
            ...
            <div className="flex flex-wrap gap-2">
              <Badge className="border-0 bg-[#f7f8f4] px-3 py-1 text-[#5b6b52] hover:bg-[#f7f8f4]">
                {totals.count} طلب معلق
              </Badge>
              <Badge className="border-0 bg-[#e9f1f3] px-3 py-1 text-[#0284C7] hover:bg-[#e9f1f3]">
                {formatCurrency(totals.amount)}
              </Badge>
              {totals.urgent > 0 && (
                <Badge className="border-0 bg-[#fdf1eb] px-3 py-1 text-[#b3694c] hover:bg-[#fdf1eb]">
                  {totals.urgent} عاجل
                </Badge>
              )}
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-[#f7f8f4]">
                  <TableHead className="text-right text-xs font-black text-[#829074]">الطلب</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">النوع</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">الأولوية</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">المبلغ</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">الإجراء</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {approvalsQuery.isLoading ? (
                  ...
                  approvalsQuery.data.map((request) => (
                    <TableRow key={request.id}>
                      <TableCell>
                        <div>
                          <p className="font-mono text-sm font-black text-[#2c4136]">{request.request_number}</p>
                          <p className="mt-1 max-w-[360px] truncate text-xs font-bold text-[#5b6b52]">{request.title}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge className="border-0 bg-[#f1f4ec] text-[#5b6b52]">
                      ...
                      <TableCell className="font-black text-[#2c4136]">{formatCurrency(request.total_amount || 0)}</TableCell>
                      <TableCell>
                        <div className="flex flex-wrap gap-2">
                          <Button
                            size="sm"
                            ...
                            className="gap-1 bg-[#2f7966] text-white hover:bg-[#256450]"
                          >
                          ...
                          <Button
                            ...
                            disabled={updateApprovalMutation.isPending}
                            className="gap-1 border-[#b3694c]/30 text-[#b3694c] hover:bg-[#fdf1eb]"
                          >
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="py-8 text-center">
                      <div className="flex flex-col items-center gap-2 text-[#2f7966]">
      ...
      <Card className="rounded-2xl border-slate-200 bg-white shadow-sm" dir="rtl">
        <CardHeader className="border-b border-slate-100 pb-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <CardTitle className="text-lg font-black text-[#2c4136]">الموافقات المتقدمة متعددة المراحل</CardTitle>
                <p className="mt-1 text-sm font-medium text-[#829074]">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Dialog open={showPolicyDialog} onOpenChange={setShowPolicyDialog}>
                <DialogTrigger asChild>
                  <Button size="sm" className="gap-1 rounded-xl bg-[#2c4136] text-white hover:bg-[#2c4136]/90">
                ...
                <DialogContent dir="rtl">
                  ...
                  <div className="space-y-4 py-2">
                    <div className="space-y-2">
                      <Label className="text-sm font-bold text-[#2c4136]">نوع الإجراء</Label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-2">
                        <Label className="text-sm font-bold text-[#2c4136]">الحد الأدنى للمبلغ</Label>
                        <Input
                          type="number"
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm font-bold text-[#2c4136]">الحد الأقصى (اختياري)</Label>
                        <Input
                          type="number"
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm font-bold text-[#2c4136]">العملة</Label>
                    ...
                    <Button
                      ...
                      className="h-11 w-full gap-2 rounded-xl bg-[#2c4136] text-white hover:bg-[#2c4136]/90"
                    >
        ...
        <CardContent className="space-y-5 p-4">
          {advancedPoliciesQuery.data && advancedPoliciesQuery.data.length > 0 && (
            <div className="rounded-2xl bg-[#f7f8f4] p-3">
              <p className="mb-2 text-xs font-black text-[#829074]">السياسات النشطة</p>
              <div className="flex flex-wrap gap-2">
                {advancedPoliciesQuery.data.filter((p) => p.is_active).map((policy) => (
                  <Badge key={policy.id} className="border-0 bg-white text-[#2c4136]">
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200">
            <Table>
              <TableHeader>
                <TableRow className="bg-[#f7f8f4]">
                  <TableHead className="text-right text-xs font-black text-[#829074]">الإجراء</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">المصدر</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">المبلغ</TableHead>
                  <TableHead className="text-right text-xs font-black text-[#829074]">الخطوة</TableHead>
              ...
              <TableBody>
                {advancedRequestsQuery.isLoading ? (
                  ...
                  advancedRequestsQuery.data.map((req) => (
                    <TableRow
                      key={req.id}
                      className={expandedRequestId === req.id ? "bg-[#f7f8f4]" : ""}
                      onClick={() => setExpandedRequestId(expandedRequestId === req.id ? null : req.id)}
                      style={{ cursor: "pointer" }}
                    >
                      <TableCell>
                        <div>
                          <p className="text-sm font-black text-[#2c4136]">
                            {ADVANCED_ACTION_LABELS[req.action] ?? req.action}
                          </p>
                          {req.notes && (
                            <p className="mt-1 max-w-[280px] truncate text-xs text-[#5b6b52]">{req.notes}</p>
                          )}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="font-black text-[#2c4136]">
                        {formatCurrency(req.amount)}
                      </TableCell>
                      <TableCell className="text-sm font-bold text-[#2c4136]">
                        ...
                        <Badge
                          className={
                            req.status === "approved"
                              ...
                              : req.status === "rejected"
                                ? "border-0 bg-[#fdf1eb] text-[#b3694c]"
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        {req.status === "pending" ? (
                          ...
                          <span className="text-xs text-[#829074]">—</span>
                        )}
                      </TableCell>
                    </TableRow>
                  ))
          ...
          {expandedRequestId && approvalActionsQuery.data && (
            <div className="rounded-2xl border border-slate-200 bg-[#f7f8f4] p-4">
              <p className="mb-3 text-xs font-black text-[#829074]">سجل إجراءات الموافقة</p>
                  ...
                  {approvalActionsQuery.data.map((act) => (
                    <div key={act.id} className="flex items-start gap-3 rounded-xl bg-white p-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                        act.action === "approved" ? "bg-[#edf4e6] text-[#2f7966]"
                        : act.action === "rejected" ? "bg-[#fdf1eb] text-[#b3694c]"
                        : "bg-[#f7f8f4] text-[#829074]"
                      }`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-bold text-[#2c4136]">
                            خطوة {act.step_order} — {act.action === "approved" ? "موافقة" : act.action === "rejected" ? "رفض" : "إلغاء"}
                          </p>
                          <p className="text-xs text-[#829074]">{new Date(act.acted_at).toLocaleString("ar")}</p>
                        </div>
                        <p className="mt-1 text-xs text-[#5b6b52]">الدور: {act.actor_role}</p>
                        {act.notes && <p className="mt-1 text-xs text-[#5b6b52]">{act.notes}</p>}
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-center text-sm text-[#829074]">لا توجد إجراءات بعد</p>
              )}
            </div>
          )}

          <div className="flex items-start gap-3 rounded-2xl bg-[#f1f4ec] p-3 text-sm font-bold text-[#4F46E5]">
