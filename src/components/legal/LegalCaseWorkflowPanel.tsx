...
export function LegalCaseWorkflowPanel({ caseId, onChanged }: Props) {
  ...
  return (
    <section className="border-y py-4 space-y-4" dir="rtl">
      ...
      {pendingTasks.length > 0 && <div className="space-y-1"><p className="text-xs font-medium text-slate-600">المتابعات الحالية</p>{pendingTasks.slice(0, 3).map((task) => <div key={task.id} className="flex items-center justify-between rounded-md border px-3 py-2 text-xs"><span>{task.title}</span><Badge variant="outline">{task.priority === 'urgent' ? 'عاجل' : 'متابعة'}</Badge></div>)}</div>}
