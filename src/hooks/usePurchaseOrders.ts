...
export interface UpdatePurchaseOrderData
  extends Omit<Partial<CreatePurchaseOrderData>, "items"> {
    ...
    mutationFn: async ({
      ...
      if (data.status) {
        if (Object.keys(data).length !== 1) {
          throw new Error(
            "Status transitions cannot be combined with purchase order edits"
          );
        }
        const { error } = await supabase.rpc(
          "transition_purchase_order_status_v1",
          {
            p_company_id: companyId,
      ...
      if (existingError || !existing)
        throw existingError || new Error("Purchase order not found");
