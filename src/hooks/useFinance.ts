...
export const usePayments = (filters?: { method?: string; status?: string }) => {
  const { companyId, user, isAuthenticating, authError } = useUnifiedCompanyAccess()
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!user?.id) {
        console.error('❌ [PAYMENTS] User not authenticated')
        throw new Error('يجب تسجيل الدخول للوصول إلى المدفوعات');
      }
      if (!companyId) {
        console.error('❌ [PAYMENTS] Company not found')
        throw new Error('معرف الشركة مطلوب');
      }

      try {
        // First try to fetch payments with basic info
        let query = supabase
          .from("payments")
          .select(`
            *,
        ...
        if (error) {
          ...
          // If foreign key constraint fails, try fetching without relations
          if (error.code === '42P01' || error.message?.includes('contracts') || error.message?.includes('customers')) {
            console.log('⚠️ [PAYMENTS] Retrying without relations due to error')
            ...
            if (fallbackError) {
              console.error('❌ [PAYMENTS] Fallback query also failed:', fallbackError)
              throw new Error(`خطأ في تحميل المدفوعات: ${fallbackError.message}`)
            }
            
            console.log('✅ [PAYMENTS] Fallback successful, loaded', fallbackData?.length || 0, 'payments without relations')
            return fallbackData || []
          }
        ...
        console.log('✅ [PAYMENTS] Successfully loaded', data?.length || 0, 'payments')
        return data || []
      } catch (error) {
    ...
    enabled: !!user?.id && !!companyId && !isAuthenticating && !authError,
    retry: (failureCount, error) => {
      // Don't retry on authentication errors
      if (error?.message?.includes('JWT') || error?.message?.includes('auth') || error?.message?.includes('تسجيل الدخول')) {
        return false
      }
