...
const migrationSQL = `-- Fix customer account linking issues
-- This migration addresses the problem where customers show "لا توجد حسابات مربوطة"

-- 1. Drop any existing versions of the function to ensure we have a clean slate
DROP FUNCTION IF EXISTS public.auto_create_customer_accounts(uuid, uuid);

-- 2. Create a standardized auto_create_customer_accounts function with proper parameter order
...
const settingsUpdateSQL = `-- 3. Ensure all companies have proper customer account settings
UPDATE companies 
...
OR NOT (customer_account_settings ? 'auto_create_account');

-- 4. Enable auto_create_account for all companies if not explicitly set
UPDATE companies 
SET customer_account_settings = jsonb_set(
    customer_account_settings,
    '{auto_create_account}',
...
const fixFunctionSQL = `-- 6. Create a function to manually fix existing customers without accounts
...
export const applyCustomerAccountMigration = async () => {
  console.log('🔄 Starting customer account migration...');
  
  try {
    // Step 1: Apply the main function
...
// Alternative approach using direct database queries
export const applyMigrationDirect = async () => {
  console.log('🔄 Applying migration using direct database queries...');
  
  try {
    // First, let's check if we need to apply the migration
    const { data: functionExists } = await supabase
      .rpc('check_function_exists', { 
        function_name: 'auto_create_customer_accounts' 
      });
    
    console.log('📊 Function exists check:', functionExists);
    
    // Update company settings directly
    console.log('⚙️ Updating company customer account settings...');
    ...
    // Enable auto_create_account for existing companies
    const { error: enableError } = await supabase
      ...
      .neq('customer_account_settings', null);
    
    if (enableError) {
      console.error('❌ Error enabling auto create:', enableError);
    } else {
      console.log('✅ Auto create enabled for all companies');
    }
