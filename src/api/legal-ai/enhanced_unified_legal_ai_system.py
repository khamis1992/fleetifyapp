#!/usr/bin/env python
"""
النظام الموحد المحسن للمستشار القانوني v2.0
يدمج الاستشارات القانونية مع الاستعلامات الذكية عن البيانات
"""
...
class EnhancedUnifiedLegalAISystem:
    ...
    def __init__(self, db_config: DatabaseConfig = None):
        # تهيئة معالج الاستفسارات العربية
        self.arabic_processor = ArabicQueryProcessor()
        
        # تهيئة محرك الاستعلامات الذكي
        if db_config:
            self.smart_engine = SmartQueryEngine(db_config)
            self.db_connector = self.smart_engine.db_connector
        else:
