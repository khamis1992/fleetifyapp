...
const ITEMS_CATALOG: CatalogItem[] = [
  { sku: "AUTO-001", description: "زيت محرك سنتيتيك 5W-30", price: 150.0 },
  { sku: "AUTO-002", description: "فلتر زيت أصلي", price: 45.0 },
  { sku: "AUTO-003", description: "فلتر هواء", price: 35.0 },
  { sku: "TIRE-001", description: "إطار ميشلان 205/55R16", price: 450.0 },
  { sku: "PART-001", description: "بطارية سيارة 12V", price: 350.0 },
];
...
export default function PurchaseOrders() {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...
        history: [
          {
            date: new Date().toISOString(),
            action: actionMap[newStatus] || "تم تحديث الحالة",
            user: "أنت",
          },
          ...prev.history,
        ],
  ...
  const ListView = () => {
    ...
    const filteredOrders =
      purchaseOrders?.filter((o) => {
        ...
        const matchesTab =
          activeTab === "الكل" || tabToStatuses[activeTab]?.includes(o.status);
    ...
    const stats = {
      ...
      openOrders:
        purchaseOrders?.filter((o) =>
          ["sent_to_vendor", "approved"].includes(o.status)
        ).length || 0,
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              هل أنت متأكد من إلغاء أمر الشراء رقم{" "}
                              {order.order_number}؟ هذا الإجراء لا يمكن التراجع
                              عنه.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
  ...
  const FormView = () => {
    ...
    const handleItemChange = (
      ...
      if (
        ["quantity", "unitPrice"].includes(field as string) ||
        field === "sku"
      ) {
        const qty =
    ...
    return (
      <div
        className="bg-[#F6F8FB] min-h-screen pb-20 print:bg-white print:pb-0"
        dir="rtl"
      >
        ...
        <div className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-sm print:hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              ...
              {formData.dbId &&
                ["sent_to_vendor", "partially_received"].includes(
                  formData.status
                ) && (
                  <button
                ...
                <table className="w-full text-right">
                  <thead>
                    <tr className="border-b-2 border-slate-800 text-slate-800">
                      ...
                      <th className="py-3 text-xs font-bold uppercase w-32">
                        كود الصنف
                      </th>
                      <th className="py-3 text-xs font-bold uppercase">
                        الوصف
                      </th>
                      <th className="py-3 text-xs font-bold uppercase w-20">
                        الكمية
                      </th>
                      <th className="py-3 text-xs font-bold uppercase w-32">
                        السعر
