...
import {
  ...
  getTaqadiFilingJobCaseSnapshot,
  ...
  syncLegalCaseWithTaqadiPayload,
...
export function TaqadiAutomationPanel({
  ...
  // The user starts the procedure once the packet is complete. The worker
  // owns review and approval after matching the live Taqadi review page.
  const requiredReady = requiredDocumentsReady && filingReadiness.canStartFiling;
  const contractIdentityBlockReason = useMemo(
    () => documents.contract.status === 'ready'
      ? getLegalContractIdentityBlockReason(documents.contract)
      : null,
    [documents.contract],
  );
  ...
  const canAutomaticallyRecoverLegacyReview = useMemo(
    ...
    [
      eventsQuery.data,
      job,
      state.taqadiData?.caseTitle,
      state.taqadiData?.contract.contractNumber,
    ],
  );
  const canAutomaticallyRecoverPartyIdentityType = useMemo(
    () => isSafePartyIdentityTypeRetry(job, eventsQuery.data ?? []),
    [eventsQuery.data, job],
  );
  const canAutomaticallyRecoverStoppedJob = canAutomaticallyRecoverLegacyReview
    || canAutomaticallyRecoverPartyIdentityType;
  ...
  useEffect(() => {
    if (
      ...
      || !requiredReady
      || contractIdentityBlockReason
      || isLegalCaseReopenable(legalCase)
      || automaticallyRetriedJobRef.current === job.id
    ) {
      return;
    }

    automaticallyRetriedJobRef.current = job.id;
    toast.info(canAutomaticallyRecoverPartyIdentityType
      ? 'يعيد الوكيل تشغيل المهمة بنوع الهوية المناسب في تقاضي'
      : 'يعيد الوكيل تحديث حزمة الدعوى القديمة تلقائياً');
  ...
  const resumeMutation = useMutation({
    mutationFn: async () => {
      ...
      const caseSnapshotMatches = currentCase.amount === refreshedPayload.case.amount
        ...
        && currentCase.title === refreshedPayload.case.title
        && currentCase.facts === refreshedPayload.case.facts
        && currentCase.claims === refreshedPayload.case.claims;

      if (!caseSnapshotMatches) {
        await syncLegalCaseWithTaqadiPayload(
          companyId,
          contractId,
          legalCase.id,
  ...
  const packageRefreshBlockReason = (() => {
    ...
    if (contractIdentityBlockReason) return contractIdentityBlockReason;
    return null;
  })();
  ...
  const startBlockReason = useMemo(() => {
    ...
    if (!requiredReady) return filingReadiness.missingReasons[0] || 'أكمل متطلبات رفع الدعوى قبل إضافتها إلى الطابور.';
    if (contractIdentityBlockReason) return contractIdentityBlockReason;
    ...
    return null;
  }, [contractIdentityBlockReason, filingReadiness.missingReasons, isActive, isCheckingStartState, job, legalCase, legalCaseAllowsStart, requiredReady]);
  ...
  return (
    <section className="lawsuit-section-panel">
      ...
      {packageRefreshBlockReason && !contractIdentityBlockReason && (
        <Alert className="mt-4 border-amber-200 bg-amber-50">
          <AlertCircle className="h-4 w-4 text-amber-700" />
          <AlertDescription className="text-amber-950">
            {packageRefreshBlockReason}
          </AlertDescription>
        </Alert>
      )}
      ...
      {contractIdentityBlockReason && (
        <Alert className="mt-4 border-red-200 bg-red-50 text-red-900">
          ...
          <AlertDescription className="flex flex-wrap items-center justify-between gap-3 pe-1">
            <span>{contractIdentityBlockReason}</span>
            <div className="flex flex-wrap items-center gap-2">
              {documents.contract.url && (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  onClick={() => window.open(documents.contract.url || '', '_blank', 'noopener,noreferrer')}
                  className="border-red-200 bg-white text-red-800 hover:bg-red-100 hover:text-red-900"
                >
              ...
              <input
                ref={contractReplacementInputRef}
                type="file"
                accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp"
                className="hidden"
                aria-label="استبدال نسخة عقد الإيجار"
                disabled={documents.contract.isUploading}
                onChange={(event) => {
                  const file = event.currentTarget.files?.[0];
                  event.currentTarget.value = '';
                  if (file) void actions.uploadDocument('contract', file);
                }}
              />
              <Button
                type="button"
                size="sm"
                onClick={() => contractReplacementInputRef.current?.click()}
                disabled={documents.contract.isUploading}
                className="bg-red-700 text-white hover:bg-red-800"
              >
                {documents.contract.isUploading
                  ? <Loader2 className="h-4 w-4 animate-spin" />
                  : <Paperclip className="h-4 w-4" />}
                {documents.contract.isUploading ? 'جارٍ الفحص' : 'استبدال نسخة العقد'}
              </Button>
            </div>
          </AlertDescription>
        </Alert>
        ...
        {job && !isActive && requiresFreshPackage && !requiresSubmissionVerification && (
          <Button
            ...
            disabled={
              ...
              || canReopenLegalCase
              || Boolean(packageRefreshBlockReason)
              || (reachedAttemptLimit && !canExplicitlyRestart)
              || !canConvertToLegal
            }
            title={!canConvertToLegal ? blockingReason : (packageRefreshBlockReason ?? undefined)}
            className="lawsuit-primary-command"
          >
            {retryMutation.isPending
              ? <Loader2 className="h-4 w-4 animate-spin" />
