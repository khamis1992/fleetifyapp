...
export default function CustomerFollowUp() {
  ...
  const [followUpForm, setFollowUpForm] = useState({
    type: 'phone' as 'phone' | 'message' | 'meeting' | 'note',
            ...
            return (
              ...
              >
                <div className="p-6">
                        ...
                        {customerFollowUps.slice(0, 2).map((followUp) => {
                          ...
                          return (
                            <div key={followUp.id} className="flex items-start gap-3">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${getFollowUpColor(followUp.type)}`}>
                  ...
                  {!needsUrgentFollowUp && customerFollowUps.some(f => f.status === 'pending') && (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-orange-700">
                            {customerFollowUps.find(f => f.status === 'pending')?.content?.slice(0, 80)}
                          </p>
                        </div>
                        <Button
                          size="sm"
                      ...
                      {customerFollowUps.length > 0 ? (
                        <div className="space-y-4 relative">
                          ...
                          {customerFollowUps.map((followUp) => {
                            const Icon = getFollowUpIcon(followUp.type);
                            const typeColor = followUp.type === 'phone' ? 'green' : followUp.type === 'message' ? 'blue' : followUp.type === 'meeting' ? 'purple' : 'gray';
                                ...
                                <div className="flex-1 bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                                  <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                      <span className="text-sm font-bold text-gray-900">
                                        {followUp.type === 'phone' ? 'مكالمة هاتفية' : 
                                         followUp.type === 'message' ? 'رسالة SMS' : 
                                         followUp.type === 'meeting' ? 'اجتماع' : 'ملاحظة'}
                                      </span>
                                    </div>
                                  ...
                                  {followUp.action_required && (
                                    <Badge variant="outline" className="bg-orange-50 text-orange-600 border-orange-200">
                                      الإجراء: {followUp.action_required}
                                    </Badge>
                                  )}
                                </div>
                              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  { value: 'note', icon: FileText, label: 'ملاحظة' },
