-- ============================================
-- إصلاح فوري للعملة والأسماء (مصحح)
-- INSTANT FIX for Currency and Names (CORRECTED)
-- ============================================
-- نسخ والصق هذا الكود في Supabase SQL Editor
-- ملاحظة: تم تصحيح استخدام الأعمدة الموجودة فعلاً
-- ============================================
...
GROUP BY c.id, c.first_name, c.last_name, c.first_name_ar, c.last_name_ar, c.company_name
LIMIT 20;
...
-- الخطوة 4: إنشاء دالة الأسماء (مصححة)
...
BEGIN
    ...
    -- البحث عن أفضل اسم متاح بالترتيب (باستخدام الأعمدة الموجودة فعلاً):
    -- 1. first_name_ar + last_name_ar (الاسم العربي)
    -- 2. company_name (اسم الشركة للعملاء من نوع شركات)
    -- 3. first_name + last_name (الاسم الإنجليزي)
    -- 4. first_name_ar فقط
    -- 5. first_name فقط
    -- 6. 'عزيزي العميل' (افتراضي)
...
BEGIN
    ...
    LOOP
        ...
        v_days_until := rec.due_date - CURRENT_DATE;
        ...
        CASE rec.reminder_type
            WHEN 'pre_due' THEN
                v_new_message := 'مرحباً ' || v_customer_name || ' 👋' || E'\n\n' ||
                    ...
                    ' ستستحق خلال ' || v_days_until || ' يوم.' || E'\n\n' ||
            ...
            ELSE
                v_new_message := 'مرحباً ' || v_customer_name || ' 👋' || E'\n\n' ||
                    'تذكير بفاتورتك رقم ' || rec.invoice_number || ' بمبلغ ' || 
                    rec.total_amount::TEXT || ' ' || v_currency_symbol || E'\n\n' ||
                    'شكراً لتعاونكم 🙏';
        END CASE;
...
SELECT 
  ...
  customer_name,
  LEFT(message_template, 150) as message_preview,
  reminder_type,
  CASE
    WHEN message_template LIKE '%ر.ق%' THEN 'ريال قطري'
    WHEN message_template LIKE '%د.ك%' THEN 'دينار كويتي'
