...
async function updateDraftToActive() {
  const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

  try {
    console.log('╔════════════════════════════════════════════╗');
    console.log('║   تحديث عقود العراف: مسودة → نشطة         ║');
    console.log('║  Update Contracts: draft → active        ║');
    console.log('╚════════════════════════════════════════════╝\n');

    // التحقق من العدد الأولي
    console.log('📊 الخطوة 1: التحقق من عدد العقود المسودة...\n');
    ...
    console.log(`✅ تم العثور على ${initialCount} عقد بحالة "مسودة"`);
    console.log(`📦 سيتم التحديث على ${Math.ceil(initialCount / BATCH_SIZE)} دفعة\n`);

    if (initialCount === 0) {
      console.log('ℹ️  لا توجد عقود مسودة تحتاج تحديث');
      return;
    }
    ...
    Object.entries(statusCounts)
      ...
      .forEach(([status, count]) => {
        ...
        const icon = status === 'active' ? '✅' : status === 'draft' ? '📝' : status === 'cancelled' ? '❌' : '📄';
