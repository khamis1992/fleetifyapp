...
const ActionButton = React.memo(({ action, index, onClick }: { 
  action: QuickAction; 
  index: number; 
...
const RealEstateQuickActions: React.FC = React.memo(() => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { hasCompanyAdminAccess, companyId } = useUnifiedCompanyAccess();

  // تصفية الإجراءات بناءً على الصلاحيات مع useMemo لمنع إعادة الحساب
  ...
  }, [hasCompanyAdminAccess, companyId]);
  ...
  }, [hasCompanyAdminAccess, companyId, navigate]);
...
RealEstateQuickActions.displayName = 'RealEstateQuickActions';
