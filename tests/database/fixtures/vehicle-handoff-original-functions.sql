CREATE OR REPLACE FUNCTION public.revert_contract_from_legal_v2(p_company_id uuid, p_contract_id uuid, p_reason text, p_idempotency_key uuid, p_actor_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE legal_case.company_id = p_company_id
      AND legal_case.contract_id = p_contract_id
      AND (
        (legal_case.filing_date IS NOT NULL AND NOT (
          legal_case.workflow_stage = 'preparation'
          AND legal_case.case_status = 'pending'
          AND legal_case.outcome_type IS NULL AND legal_case.outcome_date IS NULL
          AND EXISTS (
            SELECT 1 FROM public.contract_operations_log conversion
            WHERE conversion.company_id = p_company_id AND conversion.contract_id = p_contract_id
              AND conversion.operation_type = 'convert_to_legal'
              AND conversion.operation_details ->> 'legal_case_id' = legal_case.id::text
              AND legal_case.filing_date IN (
                (conversion.performed_at AT TIME ZONE 'Asia/Qatar')::date,
                (conversion.performed_at AT TIME ZONE 'UTC')::date
              )
          )
        ))
        OR NULLIF(BTRIM(COALESCE(legal_case.complaint_number, '')), '') IS NOT NULL
  ...
  -- Legacy conversion wrote CURRENT_DATE while still in preparation.
  -- Only recorded conversion provenance permits that date; court evidence blocks.
  IF EXISTS (
    SELECT 1 FROM public.legal_cases lc
    WHERE lc.company_id = p_company_id AND lc.contract_id = p_contract_id
      AND (EXISTS (SELECT 1 FROM public.legal_case_hearings h WHERE h.company_id=p_company_id AND h.case_id=lc.id)
        OR EXISTS (SELECT 1 FROM public.legal_case_appeals a WHERE a.company_id=p_company_id AND a.case_id=lc.id)
        OR EXISTS (SELECT 1 FROM public.legal_case_enforcements e WHERE e.company_id=p_company_id AND e.case_id=lc.id))
  ) OR EXISTS (
    SELECT 1 FROM public.taqadi_filing_artifacts a
    JOIN public.taqadi_filing_jobs j ON j.id=a.job_id AND j.company_id=a.company_id
    WHERE j.company_id=p_company_id AND j.contract_id=p_contract_id
      AND a.artifact_type IN ('receipt','submission_summary')
  ) THEN
    RAISE EXCEPTION 'توجد أدلة قضائية مرتبطة بالعقد؛ استخدم مسار إغلاق القضية الموثق' USING ERRCODE='P0001';
  END IF;
