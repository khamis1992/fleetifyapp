-- Read-only production function snapshot, 2026-09-04. Isolated test use only.
-- These are legacy definitions under audit, not proposed implementation.
CREATE OR REPLACE FUNCTION public.create_bank_transaction_journal_entry_from_record(transaction_record bank_transactions)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
