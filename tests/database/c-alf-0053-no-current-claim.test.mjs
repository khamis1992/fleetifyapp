...
test('C-ALF-0053 closure preserves paid invoices, rejects stale evidence and prevents schedule recreation',async()=>{
 const db=new PGlite();
 try{
  ...
  const beforeReceipts=(await db.query('SELECT to_jsonb(p) row FROM payments p ORDER BY id')).rows;
  const apply=await read('../../supabase/migrations/20260906185707_resolve_c_alf_0053_no_current_claim.sql');
  await db.exec("UPDATE contract_payment_schedules SET amount=1651 WHERE due_date='2025-12-01'");
  await assert.rejects(db.exec(apply),/Cancellation evidence is no longer identical/);
  await db.exec('ROLLBACK');
  assert.equal((await db.query("SELECT count(*)::int n FROM financial_data_repair_snapshots WHERE migration_version='20260906185414'")).rows[0].n,0);
  assert.equal((await db.query("SELECT count(*)::int n FROM contract_payment_schedules WHERE financial_hold_reason='cancelled_invoice_obligation_review'")).rows[0].n,17);
  await db.exec("UPDATE contract_payment_schedules SET amount=1650 WHERE due_date='2025-12-01'");
  await db.exec(await read('../../supabase/migrations/20260906185707_resolve_c_alf_0053_no_current_claim.sql'));
  ...
  assert.equal(state.canonical_paid,11550);assert.equal(state.original_remaining,28050);
  assert.equal(state.outstanding,0);assert.equal(state.review_amount,0);assert.deepEqual(state.issues,[]);
  assert.equal((await db.query("SELECT count(*)::int n FROM contract_payment_schedules WHERE financial_hold_reason='resolved_no_current_claim' AND status='cancelled'")).rows[0].n,17);
  assert.deepEqual((await db.query('SELECT to_jsonb(i) row FROM invoices i ORDER BY id')).rows,before);
  assert.deepEqual((await db.query('SELECT to_jsonb(p) row FROM payments p ORDER BY id')).rows,beforeReceipts);
  const probeMonth='2025-05-01';
  await assert.rejects(db.query("INSERT INTO contract_payment_schedules(company_id,contract_id,due_date,amount,paid_amount,status) VALUES($1,$2,$3,1650,0,'pending')",
    [f.contract.company_id,f.contract.id,probeMonth]),/مراجعة موثقة/);
  ...
  await db.exec(await read('../../supabase/rollbacks/20260906185707_resolve_c_alf_0053_no_current_claim.rollback.sql'));
  assert.equal((await db.query("SELECT count(*)::int n FROM contract_payment_schedules WHERE financial_hold_reason='cancelled_invoice_obligation_review' AND status<>'cancelled'")).rows[0].n,17);
