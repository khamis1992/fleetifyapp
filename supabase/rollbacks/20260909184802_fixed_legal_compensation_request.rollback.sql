...
DO $guard$ BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='legal_memo_calc_private.claim_register(uuid,uuid,date,jsonb)'::regprocedure) IS DISTINCT FROM 'f9a1e4f8b6de3393db8bc6e6548a0e2b' THEN RAISE EXCEPTION 'Claim register changed; review the migration before applying'; END IF;
END $guard$;
...
-- Retain the opt-in field as historical user input; the old calculator ignores it.
COMMIT;
