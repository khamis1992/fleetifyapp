-- Compensating rollback: reverse replacement invoices and issue new documents at prior values.
-- Never deletes a posted journal or marks an old reversed journal as posted again.
BEGIN;
...
DO $undo$
DECLARE x record;r jsonb;invoice_number text;
BEGIN
 FOR x IN SELECT * FROM public.financial_data_repair_snapshots WHERE migration_version='20260906180050' AND rolled_back_at IS NULL ORDER BY entity_id FOR UPDATE LOOP
  SELECT i.invoice_number INTO STRICT invoice_number FROM public.invoices i WHERE i.company_id=x.company_id AND i.id=(x.metadata->'correction'->>'replacement_invoice_id')::uuid;
  r:=pg_temp.reissue_reviewed_unpaid_rental_invoice(x.company_id,(x.before_value->>'contract_id')::uuid,invoice_number,1700,(x.before_value->>'total_amount')::numeric,
   'تراجع محاسبي موثق عن تصحيح الأجرة؛ إعادة مبلغ الفاتورة السابق مع بقاء جميع القيود والعكس في السجل.');
  UPDATE public.contract_payment_schedules SET financial_hold_reason='rental_invoice_origin_review',updated_at=now()
  WHERE company_id=x.company_id AND id=(r->'after_schedule'->>'id')::uuid;
  UPDATE public.financial_data_repair_snapshots SET rolled_back_at=now(),metadata=metadata||jsonb_build_object('compensating_rollback',r)
  WHERE migration_version=x.migration_version AND entity_type=x.entity_type AND entity_id=x.entity_id;
 END LOOP;
END;$undo$;
COMMIT;
