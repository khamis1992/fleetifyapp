...
DROP FUNCTION public.preview_contract_schedule_closure_v1(uuid,uuid),public.close_contract_schedule_closure_v1(uuid,uuid,text,uuid,text);
DROP FUNCTION contract_finance_private.preview_schedule_closure_preserving_claims(uuid,uuid),contract_finance_private.close_schedule_closure_preserving_claims(uuid,uuid,text,uuid,text);
...
END;$function$
;
DROP FUNCTION contract_finance_private.schedule_closure_snapshot(uuid,uuid,text);
-- Keep approved closure records and protected billing months; rollback never reactivates claims.
