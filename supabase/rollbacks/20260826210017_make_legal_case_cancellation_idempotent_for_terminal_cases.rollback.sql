...
BEGIN
  ...
  END IF;
  IF COALESCE(cardinality(p_case_ids), 0) = 0 OR NULLIF(BTRIM(COALESCE(p_reason, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Case identifiers and cancellation reason are required' USING ERRCODE = 'P0001';
  END IF;
  IF EXISTS (
    SELECT 1 FROM unnest(p_case_ids) requested(id)
    LEFT JOIN public.legal_cases legal_case
      ON legal_case.id = requested.id AND legal_case.company_id = p_company_id
    WHERE legal_case.id IS NULL
  ) THEN
    RAISE EXCEPTION 'One or more legal cases are outside the current company' USING ERRCODE = '42501';
  END IF;

  FOR v_case IN
    SELECT * FROM public.legal_cases
    WHERE company_id = p_company_id AND id = ANY(p_case_ids)
  ...
  LOOP
    IF lower(COALESCE(v_case.case_status, '')) <> 'cancelled' THEN
      ...
      SET case_status = 'cancelled',
          ...
          outcome_notes = concat_ws(E'\n', NULLIF(outcome_notes, ''), BTRIM(p_reason)),
          updated_at = now()
      WHERE id = v_case.id AND company_id = p_company_id;

      INSERT INTO public.legal_case_activities(
        case_id, company_id, activity_type, activity_title,
        activity_description, created_by
      ) VALUES (
        v_case.id, p_company_id, 'case_cancelled', 'تم إلغاء القضية',
        'سبب الإلغاء: ' || BTRIM(p_reason), v_actor_id
      );
      v_count := v_count + 1;
    END IF;
    ...
    IF v_case.contract_id IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM public.legal_cases other_case
    ...
    THEN
      ...
      SELECT public.system_agent_vehicle_derived_state(contract.vehicle_id, p_company_id)
      INTO v_state
      ...
      IF v_target IS NOT NULL
         AND EXISTS (
           SELECT 1 FROM unnest(enum_range(NULL::public.vehicle_status)) AS allowed(status)
