DROP FUNCTION IF EXISTS public.remember_excel_import_contract_match_v1(uuid, text, uuid, text, text, text);
DROP TABLE IF EXISTS public.excel_import_contract_match_memory;
...
BEGIN
  ...
  IF v_actor_role <> 'service_role' THEN
    IF v_actor IS NULL OR NOT (
      ...
      OR EXISTS (
        ...
        WHERE role_row.user_id = v_actor AND role_row.role = 'super_admin'
      )
    ) THEN
  ...
  SELECT version.contract_id INTO v_contract_id
