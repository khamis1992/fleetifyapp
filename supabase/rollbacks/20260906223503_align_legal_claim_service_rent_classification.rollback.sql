-- Restore reviewed calculator definitions only; preserve invoices, payments and filings.
BEGIN;
DO $restore$
BEGIN
  IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.calculate_legal_claim_breakdown_v3(uuid,uuid,date)'::regprocedure) <> '5c5211093c5cd8d6fae58618cde44462'
     OR (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.calculate_legal_claim_statement_v4(uuid,uuid,date,text,uuid[])'::regprocedure) <> '5ca5b12767113a97b0e841198b878357' THEN
    RAISE EXCEPTION 'Legal claim calculator changed after classification repair; refusing rollback';
  END IF;
  EXECUTE $original_v3$CREATE OR REPLACE FUNCTION public.calculate_legal_claim_breakdown_v3(p_company_id uuid, p_contract_id uuid, p_as_of_date date DEFAULT ((CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Qatar'::text))::date)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
...
$function$
$original_v3$;
  EXECUTE $original_v4$CREATE OR REPLACE FUNCTION public.calculate_legal_claim_statement_v4(p_company_id uuid, p_contract_id uuid, p_as_of_date date DEFAULT ((CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Qatar'::text))::date, p_claim_scope text DEFAULT 'full_outstanding'::text, p_excluded_invoice_ids uuid[] DEFAULT ARRAY[]::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SET search_path TO 'pg_catalog', 'public'
