-- Make legal-transfer permission explicit for employee-workspace users.
-- Some production accounts have profile.role = employee and user_roles.role = employee;
-- both should be valid when the user belongs to the company, especially when the
-- contract is assigned to their profile.
  ...
  SELECT
    ...
    OR (
      ...
      AND (
        ...
        OR EXISTS (SELECT 1 FROM current_profile)
      )
      AND (
        EXISTS (
          SELECT 1
          ...
          WHERE role.user_id = auth.uid()
            ...
            AND role.role::text IN (
              ...
              'employee', 'collection_agent', 'مساحة العمل فقط'
            )
        )
        OR EXISTS (
          SELECT 1
...
IS 'Allows legal-transfer preparation/conversion for privileged users, employee-workspace users in the company, and the profile assigned to the target contract.';
