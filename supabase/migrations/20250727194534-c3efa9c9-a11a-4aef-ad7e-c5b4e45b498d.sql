-- Fix the ambiguous column reference issue in create_customer_financial_account function
...
BEGIN
    ...
    -- Generate unique account code with proper table alias
    SELECT COALESCE(MAX(CAST(SUBSTRING(coa_existing.account_code FROM '[0-9]+$') AS integer)), 0) + 1
    INTO next_number
    FROM public.chart_of_accounts coa_existing
    WHERE coa_existing.company_id = company_id_param
    AND coa_existing.account_code ~ '^[0-9]+$';
    ...
    -- Ensure account code is unique with explicit table alias
    WHILE EXISTS(
        SELECT 1 
        FROM public.chart_of_accounts coa_check 
        WHERE coa_check.company_id = company_id_param 
        AND coa_check.account_code = new_account_code
    ) LOOP
    ...
    INSERT INTO public.chart_of_accounts (
        ...
        CASE 
            WHEN parent_account_id IS NOT NULL THEN
                (SELECT coa_parent.account_level + 1 
                 FROM public.chart_of_accounts coa_parent 
                 WHERE coa_parent.id = parent_account_id)
            ELSE 2 
        END,
        false,
        false,
...
-- Add unique constraint to prevent duplicate customer accounts
