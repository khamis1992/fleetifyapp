...
-- 1. Drop and recreate security definer functions with proper search paths
DROP FUNCTION IF EXISTS public.has_role(uuid, user_role);
DROP FUNCTION IF EXISTS public.get_user_company(uuid);

CREATE FUNCTION public.has_role(_user_id uuid, role_name user_role)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
    RETURN EXISTS (
        ...
        WHERE ur.user_id = _user_id AND ur.role = role_name
    );
END;
$$;

CREATE FUNCTION public.get_user_company(_user_id uuid)
RETURNS uuid
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
