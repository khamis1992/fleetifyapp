...
BEGIN
    ...
    -- Get vehicle name for description (using plate_number instead of license_plate)
...
-- Create maintenance cost summary view (fixed column name)
...
SELECT 
    ...
    MAX(vm.completed_at) as last_maintenance_date
FROM public.vehicle_maintenance vm
...
-- Default maintenance account mappings for common types
INSERT INTO public.default_account_types (type_code, type_name, type_name_ar, account_type, category, description, is_essential) 
VALUES 
('MAINTENANCE_EXPENSE', 'Vehicle Maintenance Expense', 'مصروفات صيانة المركبات', 'expense', 'fleet', 'Account for tracking vehicle maintenance costs', false),
('PARTS_EXPENSE', 'Vehicle Parts Expense', 'مصروفات قطع غيار المركبات', 'expense', 'fleet', 'Account for tracking vehicle parts costs', false),
('FUEL_EXPENSE', 'Fuel Expense', 'مصروفات الوقود', 'expense', 'fleet', 'Account for tracking fuel costs', false)
