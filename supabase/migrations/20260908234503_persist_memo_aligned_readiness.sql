-- Uses the deployed receipt/service-period memo engine. No migration-time financial DML.
BEGIN;
...
BEGIN
  FOR r IN SELECT * FROM (VALUES
    ('complete_legal_transfer_readiness_v1_pre_pdf_request_agent','uuid,uuid,jsonb,uuid','bce8a7542ebc1b3a5cd5585c3dae5cd1','before_memo_completion_bottom_v1'),
    ('complete_legal_transfer_readiness_with_scope_v1','uuid,uuid,jsonb,text,uuid','d1c3dc92014e40cc0e292daf87b2eb78','before_memo_completion_scope_v1'),
    ('complete_legal_transfer_readiness_v2','uuid,uuid,jsonb,text,uuid','5284fa39784cfe5d3bb512d784bdacdd','before_memo_completion_v2')
    ...
    EXECUTE replace(pg_get_functiondef(to_regprocedure('public.'||r.name||'('||r.args||')')),
      'public.'||r.name||'(', 'legal_memo_calc_private.'||r.backup||'(');
    EXECUTE format('REVOKE ALL ON FUNCTION legal_memo_calc_private.%I(%s) FROM PUBLIC,anon,authenticated,service_role',r.backup,r.args);
  END LOOP;
END;
$baseline$;
...
CREATE FUNCTION legal_memo_calc_private.authorize_memo_completion(
...
CREATE FUNCTION legal_memo_calc_private.prepare_memo_readiness(
...
BEGIN
  v_actor:=legal_memo_calc_private.authorize_memo_completion(p_company_id,p_contract_id,p_actor_id);
  ...
  IF v_statement->>'settlement_source' IS DISTINCT FROM 'completed_receipt_allocations_v1'
    OR v_statement->>'service_period_version' IS DISTINCT FROM 'invoice_coverage_v1' THEN
    RAISE EXCEPTION 'Canonical claim reader must be installed before readiness completion' USING ERRCODE='P0001';
  END IF;
  -- Compare before the audit INSERT. A changed claim must be reviewed again,
  -- even if the grand total stayed equal while receipts or components moved.
  IF jsonb_typeof(p_payload->'reviewed_claim_statement') IS DISTINCT FROM 'object'
    OR ((p_payload->'reviewed_claim_statement') - 'excluded_invoice_ids')
      IS DISTINCT FROM (v_statement - 'excluded_invoice_ids')
    OR p_payload->'claim_amount' IS DISTINCT FROM v_statement->'total'
  THEN
    RAISE EXCEPTION 'تغيرت المطالبة أو لم تُرسل النسخة التي تمت مراجعتها؛ أعد فحص المبالغ قبل الاعتماد'
      USING ERRCODE='40001',HINT='LEGAL_READINESS_REVIEW_CHANGED';
  END IF;
  ...
  v_saved:=(p_payload-'completed_payments'-'excluded_invoices'-'reviewed_claim_statement') || jsonb_build_object(
    'ready',true,'readiness_source','memo_readiness_completion_v1',
...
CREATE FUNCTION legal_memo_calc_private.persist_memo_readiness(
  p_company_id uuid,p_contract_id uuid,p_payload jsonb,p_actor_id uuid
) RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=''
AS $persist_command$
DECLARE v_saved jsonb;
BEGIN
  v_saved:=legal_memo_calc_private.prepare_memo_readiness(p_company_id,p_contract_id,p_payload,p_actor_id);
...
CREATE FUNCTION legal_memo_calc_private.dispatch_memo_readiness(
...
BEGIN
  PERFORM legal_memo_calc_private.authorize_memo_completion(p_company_id,p_contract_id,p_actor_id);
...
REVOKE ALL ON FUNCTION legal_memo_calc_private.authorize_memo_completion(uuid,uuid,uuid) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION legal_memo_calc_private.prepare_memo_readiness(uuid,uuid,jsonb,uuid) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION legal_memo_calc_private.persist_memo_readiness(uuid,uuid,jsonb,uuid) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION legal_memo_calc_private.dispatch_memo_readiness(uuid,uuid,jsonb,text,uuid) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION legal_memo_calc_private.persist_memo_readiness(uuid,uuid,jsonb,uuid) TO authenticated,service_role;
GRANT EXECUTE ON FUNCTION legal_memo_calc_private.dispatch_memo_readiness(uuid,uuid,jsonb,text,uuid) TO authenticated,service_role;
...
AS $facade$ SELECT legal_memo_calc_private.persist_memo_readiness(p_company_id,p_contract_id,p_payload,p_actor_id); $facade$;
...
AS $facade$ SELECT legal_memo_calc_private.dispatch_memo_readiness(p_company_id,p_contract_id,p_payload,p_claim_scope,p_actor_id); $facade$;
