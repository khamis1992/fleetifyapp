...
-- 1. Single payment amounts don't exceed 10x the monthly amount or QAR 50,000
-- 2. Total payments don't exceed the contract amount by more than 10%
...
-- View contracts with no contract amount set
SELECT
    contract_number,
    total_paid,
    contract_amount
FROM contract_payment_health_dashboard
WHERE contract_amount = 0 AND total_paid > 0
