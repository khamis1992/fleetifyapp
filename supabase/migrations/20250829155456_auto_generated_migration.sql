...
-- Company admins and managers can manage reports in their company (using company_id directly)
CREATE POLICY "2_company_admin_manage_vehicle_condition_reports" 
ON public.vehicle_condition_reports
FOR ALL
TO authenticated
USING (
  -- Allow if user is company admin/manager and the report belongs to their company
  (has_role(auth.uid(), 'company_admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role))
  AND vehicle_condition_reports.company_id = get_user_company(auth.uid())
)
WITH CHECK (
  (has_role(auth.uid(), 'company_admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role))
  AND vehicle_condition_reports.company_id = get_user_company(auth.uid())
...
TO authenticated
USING (
  vehicle_condition_reports.company_id = get_user_company(auth.uid())
);
