-- Correct the captured production rental core without changing its privileges or invoice type.
...
BEGIN
  ...
  ELSIF strpos(v_definition, v_new) > 0
    ...
    RETURN;
  ELSE
    RAISE EXCEPTION 'Rental invoice core changed after review; refusing patch';
  END IF;
END;
$migration$;
COMMIT;
