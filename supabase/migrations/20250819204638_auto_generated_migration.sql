-- Fix ambiguous column reference in enhanced_cascade_delete_account function
...
-- Recreate the function with the correct 4-parameter signature and fixed ambiguous column reference
...
DECLARE
    ...
    affected_records_data JSONB := '{}'::JSONB; -- Renamed to avoid ambiguity
...
BEGIN
    ...
    IF force_delete AND child_accounts_count > 0 THEN
        ...
        affected_records_data := affected_records_data || jsonb_build_object('child_accounts_deleted', child_accounts_count);
    END IF;
    
    -- Delete the main account
    DELETE FROM chart_of_accounts WHERE id = account_id_param;
    
    -- Update deletion log with affected records (fixed ambiguous reference)
    UPDATE account_deletion_log 
    SET affected_records = affected_records_data
