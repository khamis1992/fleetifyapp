-- إصلاح دالة الحذف المتقدم للحسابات مع معالجة قيود قاعدة البيانات
...
BEGIN
    ...
    BEGIN
        FOR table_info IN 
            ...
            WHERE kcu.referenced_table_name = 'chart_of_accounts' 
            AND kcu.referenced_column_name = 'id'
            AND kcu.table_schema = 'public'
        LOOP
            EXECUTE format('SELECT COUNT(*) FROM %I WHERE %I = $1', table_info.table_name, table_info.column_name)
            INTO count_value
            USING account_id_param;
            
            IF count_value > 0 THEN
                linked_tables := array_append(linked_tables, table_info.table_name);
                linked_tables_count := linked_tables_count + count_value;
                table_counts := jsonb_set(table_counts, ARRAY[table_info.table_name], to_jsonb(count_value));
            END IF;
        END LOOP;
    END;
    ...
    IF force_delete AND array_length(linked_tables, 1) > 0 THEN
        ...
        BEGIN
            FOREACH table_name IN ARRAY linked_tables
            LOOP
                -- حذف أو تعطيل السجلات المرتبطة حسب الجدول
                CASE table_name
                    WHEN 'customers' THEN
                ...
                IF delete_count > 0 THEN
                    affected_records := jsonb_set(affected_records, ARRAY[table_name || '_updated'], to_jsonb(delete_count));
                END IF;
            END LOOP;
        END;
    END IF;
