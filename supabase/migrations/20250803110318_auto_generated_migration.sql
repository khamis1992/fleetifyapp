-- Fix the payment schedule function to properly handle first payment date and intervals
CREATE OR REPLACE FUNCTION public.create_payment_schedule_invoices(
    p_contract_id uuid, 
    p_installment_plan text DEFAULT 'monthly'::text, 
    p_number_of_installments integer DEFAULT NULL::integer,
    p_first_payment_date date DEFAULT NULL::date
)
...
BEGIN
    ...
    -- Set the first payment date - THIS IS THE KEY FIX
    IF p_first_payment_date IS NOT NULL THEN
        next_due_date := p_first_payment_date;
    ELSE
        -- Default: start date plus the installment interval to create proper gaps
        CASE p_installment_plan
            WHEN 'monthly' THEN
                next_due_date := contract_record.start_date + INTERVAL '1 month';
            WHEN 'quarterly' THEN
                next_due_date := contract_record.start_date + INTERVAL '3 months';
            WHEN 'semi_annual' THEN
                next_due_date := contract_record.start_date + INTERVAL '6 months';
            WHEN 'annual' THEN
                next_due_date := contract_record.start_date + INTERVAL '1 year';
            ELSE
                next_due_date := contract_record.start_date + INTERVAL '1 month';
        END CASE;
    END IF;
    ...
    FOR i IN 1..installment_count LOOP
        ...
        -- Calculate next due date based on plan - THIS IS CRUCIAL FOR PROPER SPACING
        CASE p_installment_plan
            WHEN 'monthly' THEN
