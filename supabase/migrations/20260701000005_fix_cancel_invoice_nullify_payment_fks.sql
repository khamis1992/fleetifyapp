-- Fix: nullify invoice_id on payments before cancelling invoice
-- The cancel_invoice_with_reversal RPC correctly blocks active payments,
-- but cancelled payments still hold FK references that prevent the invoice UPDATE.
-- This migration nullifies those references to allow cancellation.
...
BEGIN
  ...
  -- Nullify invoice_id on all payments referencing this invoice
  -- so the FK constraint doesn't block the invoice UPDATE.
  -- Payments are preserved for audit trail; only the FK link is severed.
...
'Cancels an invoice after confirming there are no active payments, nullifying payment FK references, reversing its invoice journal entry, and preserving the audit trail.';
