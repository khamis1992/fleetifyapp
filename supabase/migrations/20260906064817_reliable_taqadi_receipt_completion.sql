-- Reliable replay of locally persisted receipts; no browser resubmission.
...
BEGIN
  ...
  END IF;
  IF p_result ? 'sourceContext' AND (
    p_result #>> '{sourceContext,companyId}' IS DISTINCT FROM v_job.company_id::text
    OR p_result #>> '{sourceContext,legalCaseId}' IS DISTINCT FROM v_job.legal_case_id::text
    OR p_result #>> '{sourceContext,contractId}' IS DISTINCT FROM v_job.contract_id::text
    OR p_result #>> '{sourceContext,memoSnapshotId}' IS DISTINCT FROM v_job.payload ->> 'memoSnapshotId'
  ) THEN
    RAISE EXCEPTION 'Saved receipt does not match the filing job' USING ERRCODE = 'P0001';
  END IF;
  IF COALESCE((v_job.payload ->> 'canary')::boolean, false) THEN
    RAISE EXCEPTION 'Canary jobs cannot complete a filing' USING ERRCODE = 'P0001';
  END IF;
  v_already_filed := v_job.status = 'filed';
  IF v_already_filed AND COALESCE(v_job.result ->> 'referenceNumber', v_job.result ->> 'caseNumber', '') <> v_reference THEN
    RAISE EXCEPTION 'Conflicting completed receipt reference' USING ERRCODE = 'P0001';
  END IF;
  IF v_job.status NOT IN ('submitting', 'filed') THEN
  ...
  END IF;
  IF NULLIF(BTRIM(v_case.case_reference), '') IS NOT NULL AND v_case.case_reference <> v_reference THEN
    RAISE EXCEPTION 'Conflicting legal case reference' USING ERRCODE = 'P0001';
  END IF;
  -- Lost responses are safe to replay, including after a later court event.
  -- Only the legacy filed stage needs to be advanced on replay.
  IF v_already_filed AND v_case.workflow_stage NOT IN ('preparation', 'filed') THEN
  ...
  WHERE company_id = v_job.company_id
    AND contract_id = v_job.contract_id
    AND (id = v_job.lawsuit_preparation_id OR legal_case_id = v_job.legal_case_id);
  GET DIAGNOSTICS v_preparation_count = ROW_COUNT;
  IF v_preparation_count = 0 THEN
    RAISE EXCEPTION 'Matching lawsuit preparation was not found' USING ERRCODE = 'P0001';
  END IF;
  ...
  SET
    ...
    progress = 100,
    result = COALESCE(v_job.result, '{}'::jsonb) || COALESCE(p_result, '{}'::jsonb) || jsonb_build_object(
      'caseNumber', NULLIF(BTRIM(COALESCE(p_case_number, '')), ''),
      'referenceNumber', v_reference,
      'courtFees', p_court_fees,
      'filedAt', COALESCE(v_job.result ->> 'filedAt', p_result ->> 'receiptCapturedAt', now()::text),
