-- assign_contract_cost_center() fills cost_center_id from the customer default
-- when the client omits it. A retry with the same idempotency key then compared
-- NULL to that assigned uuid and raised 23505 even though the request matched.
...
BEGIN
  ...
  IF v_def NOT LIKE '%OR v_existing_contract.cost_center_id IS DISTINCT FROM p_cost_center_id%' THEN
    RAISE EXCEPTION 'Expected cost_center replay comparison was not found';
  END IF;
