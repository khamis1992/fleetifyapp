-- Apply a complete schedule-to-invoice matching that includes unlinked schedules.
...
INSERT INTO public.system_agent_command_registry (
  ...
  'schedule.realign_contract_invoice_links_v3', 'contracts',
  'Apply a database-validated one-to-one matching across every active schedule in one contract, including currently unlinked schedules.',
...
CREATE OR REPLACE FUNCTION public.system_agent_apply_contract_schedule_matching_repair_v3(
  p_run_id uuid,
  p_job_id uuid,
  p_finding_id uuid,
  p_command text,
...
DECLARE
  ...
  v_rollback_metadata jsonb := COALESCE(p_metadata, '{}'::jsonb)
    || jsonb_build_object('handler_version', 'contract_schedule_matching_v3');
BEGIN
  IF p_command <> 'schedule.realign_contract_invoice_links_v3' THEN
  ...
  IF COALESCE((p_expected_before ->> 'active_schedule_count')::integer, v_active_schedule_count)
     ...
     OR v_assignment_count IS DISTINCT FROM v_active_schedule_count
  THEN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Contract schedule matching does not cover the current active schedule graph';
  END IF;
...
REVOKE ALL ON FUNCTION public.system_agent_apply_contract_schedule_matching_repair_v3(uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.system_agent_apply_contract_schedule_matching_repair_v3(uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb)
  TO service_role;

DO $$
BEGIN
  IF to_regprocedure('public.system_agent_rollback_repair_before_contract_schedule_matching_v3(uuid,text)') IS NULL THEN
    ALTER FUNCTION public.system_agent_rollback_repair(uuid, text)
      RENAME TO system_agent_rollback_repair_before_contract_schedule_matching_v3;
  END IF;
END;
$$;

REVOKE ALL ON FUNCTION public.system_agent_rollback_repair_before_contract_schedule_matching_v3(uuid,text)
  FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF COALESCE(v_repair.rollback_metadata ->> 'handler_version', '') <> 'contract_schedule_matching_v3' THEN
    RETURN public.system_agent_rollback_repair_before_contract_schedule_matching_v3(p_repair_id, p_reason);
  END IF;
...
COMMENT ON FUNCTION public.system_agent_apply_contract_schedule_matching_repair_v3(uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb) IS
'Canonical database-validated one-to-one contract schedule invoice matching for every active schedule, including previously unlinked schedules, with full-state rollback.';
