-- Prevent legal claims from counting traffic penalties twice.
--
-- The previous breakdown treated every due invoice as recorded rent/receivable,
-- including service invoices linked to penalties. When a violations-proof
-- document existed, the same penalties were then added again from `penalties`.
-- This replacement keeps only rental sales invoices (plus uncovered rental
-- schedules) in recorded_due_amount and reports excluded invoice components
...
COMMENT ON FUNCTION public.calculate_legal_claim_breakdown_v3(UUID, UUID, DATE) IS
  'Evidence-aware legal claim breakdown. recorded_due_amount contains rent-only sales invoices and uncovered rental schedules; penalty-linked and other service invoices are excluded and disclosed separately so evidenced violations cannot be counted twice.';
