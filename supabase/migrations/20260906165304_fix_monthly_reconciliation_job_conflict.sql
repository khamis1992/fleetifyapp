BEGIN;
DO $$ BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='public.monthly_contract_invoice_reconciliation(date)'::regprocedure) IS DISTINCT FROM '16934799ba83959abc699900f96be5b1' THEN
  RAISE EXCEPTION 'Monthly reconciliation function changed since inspection'; END IF;
END; $$;
CREATE OR REPLACE FUNCTION public.monthly_contract_invoice_reconciliation(p_target_month date DEFAULT (date_trunc('month'::text, (CURRENT_DATE + '1 mon'::interval)))::date)
 RETURNS TABLE(company_id uuid, contract_id uuid, contract_number text, invoice_month date, action text, invoice_id uuid, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT ON CONSTRAINT system_agent_jobs_run_id_company_id_domain_key DO UPDATE
    SET updated_at = now()
