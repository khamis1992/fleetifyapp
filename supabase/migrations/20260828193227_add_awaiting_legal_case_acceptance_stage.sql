-- Insert a distinct court-acceptance wait state between filing and hearings.
-- The workflow remains text-backed, so the table constraint and every guarded
-- transition entry point must be updated together.
...
BEGIN
  ...
  SET
    legal_status = CASE p_stage
      ...
      WHEN 'awaiting_acceptance' THEN 'legal_case_filed'
...
BEGIN
  ...
  v_allowed := CASE v_case.workflow_stage
    ...
    WHEN 'filed' THEN p_target_stage IN ('awaiting_acceptance', 'cancelled')
    WHEN 'awaiting_acceptance' THEN p_target_stage IN ('hearings', 'cancelled')
  ...
  IF NOT v_allowed THEN
    RAISE EXCEPTION 'Invalid legal workflow transition from % to %', v_case.workflow_stage, p_target_stage
      USING ERRCODE = 'P0001';
  END IF;
  IF p_target_stage IN ('cancelled', 'closed')
  ...
  v_legacy := CASE
    WHEN p_target_stage = 'preparation' THEN 'pending'
    WHEN p_target_stage IN ('closed', 'cancelled') THEN p_target_stage
    ELSE 'active'
  END;
  ...
  SET
    ...
    filing_date = CASE
      WHEN p_target_stage = 'filed' THEN COALESCE(filing_date, CURRENT_DATE)
      ELSE filing_date
    END,
    closed_at = CASE WHEN p_target_stage = 'closed' THEN now() ELSE NULL END,
    closure_reason = CASE
      WHEN p_target_stage IN ('closed', 'cancelled') THEN BTRIM(p_reason)
      ELSE closure_reason
    END,
    updated_at = now()
  WHERE id = p_case_id
  ...
  WHERE company_id = p_company_id
    ...
    AND (
      (v_old_stage = 'filed' AND metadata->>'workflow_key' = 'acceptance-state:' || p_case_id::text)
      OR (v_old_stage = 'awaiting_acceptance' AND metadata->>'workflow_key' = 'court-acceptance:' || p_case_id::text)
    );
  ...
  INSERT INTO public.legal_case_activities(
    ...
    'workflow_transition',
    'تغيير مرحلة القضية',
    COALESCE(NULLIF(BTRIM(p_reason), ''), 'انتقال معتمد ضمن سير العمل'),
    jsonb_build_object('workflow_stage', v_old_stage),
    jsonb_build_object('workflow_stage', p_target_stage),
    v_actor
  );
...
BEGIN
  v_actor := public.legal_workflow_actor_profile_v1(p_company_id, p_actor_id);
  IF p_hearing_date IS NULL OR p_status NOT IN ('scheduled', 'completed', 'adjourned', 'cancelled') THEN
    RAISE EXCEPTION 'Valid hearing date and status are required';
  END IF;
  ...
  IF v_case.workflow_stage NOT IN ('awaiting_acceptance', 'hearings') THEN
    RAISE EXCEPTION 'Court acceptance must be recorded before the first hearing'
      USING ERRCODE = 'P0001';
  END IF;

  INSERT INTO public.legal_case_hearings(
    company_id,
    case_id,
    hearing_date,
    status,
  ...
  UPDATE public.legal_cases
  SET
    workflow_stage = 'hearings',
    case_status = 'active',
    hearing_date = COALESCE(p_next_hearing_date, p_hearing_date),
    stage_updated_at = now(),
    updated_at = now()
  WHERE id = p_case_id;
  ...
  IF COALESCE(p_next_hearing_date, CASE WHEN p_status = 'scheduled' THEN p_hearing_date END) IS NOT NULL THEN
    PERFORM public.legal_workflow_create_task_v1(
      p_company_id,
      p_case_id,
      'hearing:' || v_hearing.id::text,
      'متابعة جلسة ' || v_case.case_number,
      'موعد جلسة القضية ومراجعة متطلباتها.',
      COALESCE(p_next_hearing_date, p_hearing_date) - interval '1 day',
      'urgent',
      v_actor
    );
  END IF;
  ...
  INSERT INTO public.legal_case_activities(
    ...
    'hearing_recorded',
    'تسجيل جلسة',
    COALESCE(p_decision, 'تم تسجيل موعد الجلسة'),
    to_jsonb(v_hearing),
    v_actor
  );
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL
     AND NOT (public.is_company_admin(p_company_id) OR public.is_company_manager(p_company_id)) THEN
    RAISE EXCEPTION 'Manager permission is required to reopen a case' USING ERRCODE = '42501';
  END IF;
  IF p_target_stage NOT IN (
    ...
    RAISE EXCEPTION 'A valid target stage and detailed reason are required';
  END IF;
  ...
  IF NOT FOUND OR v_case.workflow_stage NOT IN ('closed', 'cancelled') THEN
    RAISE EXCEPTION 'Only closed or cancelled cases can be reopened';
  END IF;
  ...
  SET
    workflow_stage = p_target_stage,
    case_status = CASE WHEN p_target_stage = 'preparation' THEN 'pending' ELSE 'active' END,
    stage_updated_at = now(),
    closed_at = NULL,
    closure_reason = NULL,
    reopened_at = now(),
...
BEGIN
  FOR v_case IN
    SELECT * FROM public.legal_cases WHERE workflow_stage NOT IN ('closed', 'cancelled')
  LOOP
    ...
    v_actor := COALESCE(
      v_actor,
      (SELECT id FROM public.profiles WHERE company_id = v_case.company_id ORDER BY created_at LIMIT 1)
    );
    ...
    IF v_case.workflow_stage = 'filed' THEN
      v_id := public.legal_workflow_create_task_v1(v_case.company_id, v_case.id, 'acceptance-state:' || v_case.id::text, 'بدء متابعة قبول الدعوى ' || v_case.case_number, 'تم رفع الدعوى؛ حدّث المرحلة إلى بانتظار قبول الدعوى بعد التحقق من إشعار الإيداع.', now() + interval '1 day', 'urgent', v_actor);
      IF v_id IS NOT NULL THEN v_checked := v_checked + 1; END IF;
    END IF;
    IF v_case.workflow_stage = 'awaiting_acceptance' AND v_case.stage_updated_at < now() - interval '3 days' THEN
      v_id := public.legal_workflow_create_task_v1(v_case.company_id, v_case.id, 'court-acceptance:' || v_case.id::text, 'متابعة قبول الدعوى ' || v_case.case_number, 'الدعوى بانتظار قبول المحكمة؛ راجع إشعار القبول أو الرفض وحدّث سجل القضية.', now() + interval '1 day', 'urgent', v_actor);
      IF v_id IS NOT NULL THEN v_checked := v_checked + 1; END IF;
    END IF;
    ...
    IF v_case.workflow_stage = 'reserved_for_judgment' AND v_case.stage_updated_at < now() - interval '7 days' THEN
      ...
      IF v_id IS NOT NULL THEN v_checked := v_checked + 1; END IF;
    END IF;
    IF v_case.appeal_deadline IS NOT NULL
       AND v_case.appeal_deadline <= CURRENT_DATE + 7
       AND v_case.workflow_stage = 'judgment_issued' THEN
...
COMMENT ON CONSTRAINT legal_cases_workflow_stage_check ON public.legal_cases IS
  'Canonical legal workflow, including court acceptance wait between filing and hearings.';
