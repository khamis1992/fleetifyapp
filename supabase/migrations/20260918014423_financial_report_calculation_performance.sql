...
CREATE FUNCTION balance_sheet_private.assemble_report(p_company uuid,p_as_of date,p_comparison date,p_current jsonb,p_comparison_source jsonb)
...
END;
$fn$;
REVOKE ALL ON FUNCTION balance_sheet_private.assemble_report(uuid,date,date,jsonb,jsonb) FROM PUBLIC,anon,authenticated,service_role;

CREATE OR REPLACE FUNCTION balance_sheet_private.calculate(p_company uuid,p_as_of date,p_comparison date)
RETURNS jsonb LANGUAGE plpgsql STABLE SET search_path = '' SET timezone = 'UTC' AS $fn$
DECLARE v_current jsonb;v_comparison jsonb;
BEGIN
  IF p_as_of IS NULL OR NOT isfinite(p_as_of) OR p_as_of < DATE '0001-01-01' OR p_as_of > (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Qatar')::date
    OR (p_comparison IS NOT NULL AND (NOT isfinite(p_comparison) OR p_comparison < DATE '0001-01-01' OR p_comparison >= p_as_of)) THEN
    RAISE EXCEPTION 'Invalid balance sheet date or comparison date' USING ERRCODE='22023';
  ...
  RETURN balance_sheet_private.assemble_report(p_company,p_as_of,p_comparison,v_current,v_comparison);
END;
$fn$;
...
DECLARE v_j record;v_l jsonb;v_m jsonb;v_o jsonb;v_a jsonb;v_income jsonb:='{}';v_equity jsonb:='{}';v_cash jsonb:='{}';v_cash_details jsonb:='{}';
 v_findings jsonb[]:=ARRAY[]::jsonb[];v_journals jsonb[]:=ARRAY[]::jsonb[];v_mapping_lookup jsonb;v_override_lookup jsonb;v_amount numeric;v_net numeric;v_cd numeric;v_cc numeric;
...
BEGIN
 -- Build constant-time key lookups once; configuration validation rejects duplicate IDs.
 SELECT COALESCE(jsonb_object_agg(m->>'accountId',m),'{}'::jsonb) INTO v_mapping_lookup FROM jsonb_array_elements(p_config->'accountMappings') m;
 SELECT COALESCE(jsonb_object_agg(o->>'journalId',o),'{}'::jsonb) INTO v_override_lookup FROM jsonb_array_elements(p_config->'journalOverrides') o;
 FOR v_j IN
 ...
 LOOP
   ...
   v_o:=v_override_lookup->(v_j.id::text);
   ...
   IF v_badclosing THEN
     v_findings:=array_append(v_findings,(financial_statement_private.finding('invalid_closing_journal','قيد الإقفال يتضمن حسابات أو حركة غير مناسبة','Closing journal contains inappropriate accounts or movements','[]',jsonb_build_array(v_j.id)))->0);
   END IF;
   IF v_j.reference_type='annual_close_opening' THEN
     v_findings:=array_append(v_findings,(financial_statement_private.finding('carryforward_requires_review','يلزم التحقق من قيد الأرصدة الافتتاحية لمنع تكرار الأرصدة','Review annual carryforward journal for duplicated cumulative balances','[]',jsonb_build_array(v_j.id)))->0);
   END IF;
   IF NOT v_closing AND v_o->>'equityCategory'='transfers' AND
     ...
     v_findings:=array_append(v_findings,(financial_statement_private.finding('invalid_equity_transfer','التحويل بين مكونات حقوق الملكية يجب ألا يغير إجماليها أو يتضمن حركة خارجية','Transfers between equity components must have zero total and only equity lines','[]',jsonb_build_array(v_j.id)))->0);
   END IF;
   FOR v_l IN SELECT value FROM jsonb_array_elements(v_j.lines) LOOP
     v_m:=v_mapping_lookup->(v_l->>'accountId');
     ...
     IF NOT v_closing AND v_l->>'type' IN ('revenue','expense') THEN
       ...
       IF v_m->>'incomeLine' IS NULL AND v_amount<>0 THEN
         v_findings:=array_append(v_findings,(financial_statement_private.finding('income_mapping_missing','يلزم تحديد بند قائمة الدخل للحساب','Income statement classification is missing',jsonb_build_array(v_l->>'accountId'),jsonb_build_array(v_j.id)))->0);
       END IF;
     ...
     IF v_l->>'type'='equity' AND v_amount<>0 THEN
       ...
       ELSE
         ...
         IF v_category IS NULL THEN
           v_eq_review:=true;
           v_findings:=array_append(v_findings,(financial_statement_private.finding('equity_movement_unclassified','يلزم تصنيف حركة حقوق الملكية وشرحها','Direct equity movement requires a reviewed category','[]',jsonb_build_array(v_j.id)))->0);
         END IF;
   ...
   IF (v_cd<>0 OR v_cc<>0) AND v_noncash>0 THEN
     ...
     IF jsonb_array_length(COALESCE(v_o->'cashFlows','[]'))>0 THEN
       ...
       IF abs(v_sum-v_net)>0.005 OR v_internal>least(v_cd,v_cc)+0.005
         ...
         v_cf_review:=true;
         v_findings:=array_append(v_findings,(financial_statement_private.finding('cash_flow_allocation_mismatch','توزيع التدفقات لا يطابق إجمالي النقد المدين والدائن','Cash flow allocations do not reconcile to actual cash debits and credits','[]',jsonb_build_array(v_j.id)))->0);
       ELSE
     ...
     ELSIF v_simple AND v_internal=0 THEN
       FOR v_l IN SELECT value FROM jsonb_array_elements(v_j.lines) LOOP
         ...
         END IF;
       END LOOP;
       IF v_cf_review THEN v_findings:=array_append(v_findings,(financial_statement_private.finding('cash_flow_mapping_missing','يلزم تصنيف الحساب المقابل للتدفق النقدي','Cash flow counterpart classification is missing','[]',jsonb_build_array(v_j.id)))->0); END IF;
     ELSE
       v_cf_review:=true;v_findings:=array_append(v_findings,(financial_statement_private.finding('complex_cash_flow_requires_allocation','القيد المختلط يحتاج توزيعاً مراجعاً للتدفقات النقدية','Mixed cash and non-cash journal requires reviewed cash allocations','[]',jsonb_build_array(v_j.id)))->0);
     END IF;
   ELSIF jsonb_array_length(COALESCE(v_o->'cashFlows','[]'))>0 OR (v_internal<>0 AND (v_noncash>0 OR abs(v_internal-least(v_cd,v_cc))>0.005)) THEN
     v_findings:=array_append(v_findings,(financial_statement_private.finding('unexpected_cash_flow_allocation','لا يجوز توزيع قيد غير نقدي أو تحويل داخلي كتدفق خارجي','Non-cash journals and internal cash transfers cannot be allocated as external cash flows','[]',jsonb_build_array(v_j.id)))->0);
   END IF;
   -- PostgreSQL expanded arrays append without re-copying the entire JSON document.
   v_journals:=array_append(v_journals,jsonb_build_object('id',v_j.id,'date',v_j.entry_date,'number',COALESCE(v_j.entry_number,''),
     ...
     'effectiveTreatment',CASE WHEN v_closing THEN 'closing' ELSE 'regular' END,'isCanonicalClosing',v_j.is_canonical_closing));
 END LOOP;
 RETURN jsonb_build_object('income',v_income,'equity',v_equity,'cash',v_cash,'cashDetails',v_cash_details,'profit',v_profit,'findings',to_jsonb(v_findings),'journals',to_jsonb(v_journals),'postedEntries',v_n);
END;
$f$;
