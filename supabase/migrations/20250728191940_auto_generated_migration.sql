-- إنشاء بيانات اختبار للتنبيهات

-- إضافة مركبات تجريبية لتفعيل تنبيهات المركبات
DO $$
DECLARE
    company_uuid uuid;
    vehicle_id uuid;
...
BEGIN
    ...
    IF company_uuid IS NOT NULL THEN
        ...
        INSERT INTO vehicles (
            ...
            purchase_cost, mileage, registration_expiry, insurance_expiry,
            created_at, updated_at
        ) VALUES 
        (gen_random_uuid(), company_uuid, 'A-123-45', 'تويوتا', 'كامري', 2020, 'active', 15000, 80000, CURRENT_DATE + INTERVAL '15 days', CURRENT_DATE + INTERVAL '10 days', now(), now()),
        (gen_random_uuid(), company_uuid, 'B-678-90', 'نيسان', 'التيما', 2019, 'active', 18000, 95000, CURRENT_DATE + INTERVAL '5 days', CURRENT_DATE + INTERVAL '30 days', now(), now()),
        (gen_random_uuid(), company_uuid, 'C-111-22', 'هوندا', 'أكورد', 2021, 'maintenance', 20000, 45000, CURRENT_DATE + INTERVAL '60 days', CURRENT_DATE + INTERVAL '45 days', now(), now())
        ...
        IF vehicle_id IS NOT NULL THEN
            ...
            (gen_random_uuid(), company_uuid, vehicle_id, 'maintenance_due', 'صيانة مستحقة', 'صيانة دورية مطلوبة للمركبة A-123-45', 'high', CURRENT_DATE + INTERVAL '3 days', false, now()),
            (gen_random_uuid(), company_uuid, vehicle_id, 'insurance_expiry', 'انتهاء التأمين', 'تأمين المركبة B-678-90 ينتهي خلال 10 أيام', 'high', CURRENT_DATE + INTERVAL '10 days', false, now()),
            (gen_random_uuid(), company_uuid, vehicle_id, 'registration_expiry', 'انتهاء التسجيل', 'تسجيل المركبة A-123-45 ينتهي خلال 15 يوم', 'medium', CURRENT_DATE + INTERVAL '15 days', false, now())
