-- Fix enhanced_cascade_delete_account to handle contracts in child accounts
...
BEGIN
    ...
    SELECT array_agg(id) INTO all_affected_accounts FROM account_tree;
    
    -- Get only child accounts (excluding the main account)
    SELECT array_agg(id) INTO child_accounts 
