-- Fix enhanced_cascade_delete_account function to handle correct table relationships
...
BEGIN
    ...
    -- Check tables that actually have account_id column
    ...
    -- 2. Payments (check if account_id column exists)
    BEGIN
    ...
    -- 4. Customer Accounts (if exists)
    BEGIN
        SELECT COUNT(*) INTO temp_count FROM public.customer_accounts WHERE account_id = account_id_param;
        IF temp_count > 0 THEN
    ...
    -- 5. Essential Account Mappings (if exists)
    BEGIN
        SELECT COUNT(*) INTO temp_count FROM public.essential_account_mappings WHERE account_id = account_id_param;
        IF temp_count > 0 THEN
    ...
    -- 6. Account Mappings (if exists)
    BEGIN
        SELECT COUNT(*) INTO temp_count FROM public.account_mappings WHERE chart_of_accounts_id = account_id_param;
        IF temp_count > 0 THEN
    ...
    IF force_delete = true THEN
        ...
        IF transfer_to_account_id IS NOT NULL THEN
            ...
            BEGIN
                ...
                WHERE chart_of_accounts_id = account_id_param;
            EXCEPTION
                WHEN undefined_table THEN
                    NULL;
            END;
        ELSE
            ...
            BEGIN
                DELETE FROM public.essential_account_mappings WHERE account_id = account_id_param;
            EXCEPTION
                WHEN undefined_table THEN
                    NULL;
            END;
            ...
            BEGIN
                DELETE FROM public.account_mappings WHERE chart_of_accounts_id = account_id_param;
            EXCEPTION
                WHEN undefined_table THEN
                    NULL;
            END;
