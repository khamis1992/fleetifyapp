-- Approval-gated apply path for contract-terms-scanner proposals.
--
-- The signed contract document is the source of truth. Once finance approves
-- the extracted terms, this command applies them transactionally:
--   1. contract_amount / monthly_amount / start_date / end_date on the contract
--   2. amounts of active payment schedules that have no payment history
--   3. invoices with no payment history: cancelled with journal reversal and
--      reissued at the written amount through the canonical generator
--   4. schedules are re-linked to their same-month invoice
--   5. contract totals are recalculated canonically
--
-- Invoices or schedules with payment history are NEVER rewritten by this
-- command; they are reported for the credit/review flow because rewriting
-- them would move money the customer already paid.
...
BEGIN
  ...
  IF COALESCE(v_total, 0) <= 0 THEN
    -- A missing written total falls back to the written monthly amount across
    -- the written period.
  ...
  -- 2) Schedules outside the written period are cancelled (only when they
  --    carry no payment history and have no active invoice).
  FOR v_schedule IN
    SELECT schedule.*
  ...
  -- 3) Unpaid schedules inside the written period adopt the written amount.
  FOR v_schedule IN
    SELECT schedule.*
    FROM public.contract_payment_schedules schedule
    WHERE schedule.contract_id = v_contract.id
  ...
  LOOP
    -- The schedule amount is the written obligation; paid_amount is received
    -- history and is preserved even when it exceeds the corrected amount (the
    -- excess surfaces as an overpayment credit, never deleted).
  ...
  -- 4) Invoices without payment history are reissued at the written amount
  --    through the canonical cancel+regenerate path, so their journals are
  --    reversed and reposted correctly. Paid invoices are reported only.
  FOR v_invoice IN
    SELECT invoice.*
...
COMMENT ON FUNCTION public.apply_contract_terms_scan_proposal(uuid, text) IS
  'Approval-gated application of signed-document contract terms: rewrites contract terms, unpaid schedules, and payment-free invoices through canonical cancel+regenerate paths; payment history is never rewritten.';
