...
BEGIN
  ...
  IF p_deadline IS NOT NULL AND p_status NOT IN ('withdrawn','expired','rejected') THEN
    PERFORM public.legal_workflow_create_task_v1(p_company_id,p_case_id,'appeal-record:'||v_appeal.id::text,'متابعة استئناف '||v_case.case_number,'متابعة ملف الاستئناف قبل انتهاء المهلة.',p_deadline::timestamptz-interval '2 days','urgent',v_actor);
  END IF;
  ...
  VALUES(p_case_id,p_company_id,'appeal_recorded','تسجيل الاستئناف',COALESCE(NULLIF(BTRIM(p_notes),''),'تم تسجيل بيانات الاستئناف'),to_jsonb(v_appeal),v_actor);
  RETURN v_appeal;
END; $$;
...
BEGIN
  ...
  IF v_case.contract_id IS NOT NULL THEN
    PERFORM public.legal_workflow_create_task_v1(p_company_id,p_case_id,'post-close-contract:'||p_case_id::text,
      'مراجعة العقد بعد إغلاق '||v_case.case_number,'القضية مغلقة قانونياً. راجع رصيد العقد وحالته التشغيلية قبل أي تغيير.',now()+interval '1 day','high',v_actor);
  END IF;
