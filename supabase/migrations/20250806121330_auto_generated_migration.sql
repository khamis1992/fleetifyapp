-- Create table for AI activity logs (using profiles table instead of company_users)
...
-- Create policies for ai_activity_logs (using profiles table)
...
CREATE INDEX IF NOT EXISTS idx_ai_activity_logs_timestamp ON public.ai_activity_logs(timestamp);
CREATE INDEX IF NOT EXISTS idx_ai_activity_logs_activity_type ON public.ai_activity_logs(activity_type);

CREATE INDEX IF NOT EXISTS idx_saved_conversations_company_id ON public.saved_conversations(company_id);
CREATE INDEX IF NOT EXISTS idx_saved_conversations_user_id ON public.saved_conversations(user_id);
CREATE INDEX IF NOT EXISTS idx_saved_conversations_created_at ON public.saved_conversations(created_at);