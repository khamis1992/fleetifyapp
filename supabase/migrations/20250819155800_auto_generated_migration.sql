-- Fix account hierarchy by properly connecting Cash accounts to Assets (company-specific)
UPDATE public.chart_of_accounts AS target
SET parent_account_id = parent_acc.id
FROM public.chart_of_accounts AS parent_acc
WHERE target.account_code IN ('1102', '1103')
  AND target.parent_account_id IS NULL
  AND parent_acc.account_code = '1000'
  AND parent_acc.company_id = target.company_id;
...
BEGIN
  ...
  IF transfer_to_account_id IS NOT NULL THEN
    ...
    -- Move child accounts to transfer target as parent
  ...
  IF force_delete THEN
    ...
    -- Update references to NULL for optional relationships
    ...
    -- Finally deactivate the main account
    UPDATE public.chart_of_accounts 
    SET is_active = false, updated_at = now() 
    WHERE id = account_id_param;
    ...
    RETURN jsonb_build_object(
      ...
      'child_accounts_deleted', deleted_children
    );
  END IF;

  -- Default case - safe delete if no constraints
  IF (array_length(linked_tables, 1) IS NULL OR array_length(linked_tables, 1) = 0) AND child_accounts_count = 0 THEN
    UPDATE public.chart_of_accounts 
    SET is_active = false, updated_at = now() 
    WHERE id = account_id_param;
