...
create or replace function public.system_agent_apply_contract_invoice_billing_month_repair_v5(
  p_run_id uuid,
  p_job_id uuid,
  p_finding_id uuid,
  p_command text,
...
begin
  ...
  v_repair_metadata := v_repair_metadata || jsonb_build_object(
    'handler_version', case
      when v_created_invoice_id is null then 'contract_schedule_v1'
...
revoke all on function public.system_agent_apply_contract_invoice_billing_month_repair_v5(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) from public, anon, authenticated;
grant execute on function public.system_agent_apply_contract_invoice_billing_month_repair_v5(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) to service_role;

comment on function public.system_agent_apply_contract_invoice_billing_month_repair_v5(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) is 'Canonical contract-level billing-mode schedule invoice repair with optimistic checks, closed-period protection, audit state, and compatible rollback metadata.';
