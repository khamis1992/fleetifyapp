-- Fix ambiguous column reference in auto_create_customer_accounts function
...
BEGIN
    ...
    LOOP
        ...
        IF account_type_record.parent_account_code IS NOT NULL THEN
            ...
            SELECT COALESCE(MAX(CAST(RIGHT(coa.account_code, 2) AS INTEGER)), 0) + 1 INTO account_code
            ...
            account_code := account_type_record.parent_account_code || LPAD(account_code::TEXT, 2, '0');
        ELSE
            -- Generate a basic account code if no parent specified
            account_code := '2' || LPAD((1000 + total_created)::TEXT, 3, '0');
        END IF;
        ...
        FROM chart_of_accounts coa
        WHERE coa.company_id = p_company_id 
        AND coa.account_code = account_code;
        ...
        IF existing_account_id IS NULL THEN
            ...
            INSERT INTO chart_of_accounts (
                ...
                true,
                CASE 
                    WHEN LENGTH(account_code) <= 4 THEN LENGTH(account_code)
                    ELSE 5 
                END,
