...
BEGIN
  ...
  IF COALESCE(p_next_hearing_date,CASE WHEN p_status='scheduled' THEN p_hearing_date END) IS NOT NULL THEN
    PERFORM public.legal_workflow_create_task_v1(p_company_id,p_case_id,'hearing:'||v_hearing.id::text,'متابعة جلسة '||v_case.case_number,
      'موعد جلسة القضية ومراجعة متطلباتها.',COALESCE(p_next_hearing_date,p_hearing_date)-interval '1 day','urgent',v_actor);
  END IF;
...
BEGIN
  ...
  IF p_appeal_deadline IS NOT NULL THEN
    INSERT INTO public.legal_case_appeals(company_id,case_id,status,deadline,notes,created_by) VALUES(p_company_id,p_case_id,'eligible',p_appeal_deadline,'مهلة الطعن على الحكم',v_actor);
    PERFORM public.legal_workflow_create_task_v1(p_company_id,p_case_id,'appeal:'||p_case_id::text||':'||p_appeal_deadline::text,
      'قرار الاستئناف للقضية '||v_case.case_number,'تنتهي مهلة الاستئناف في التاريخ المحدد.',p_appeal_deadline::timestamptz-interval '3 days','urgent',v_actor);
  END IF;
  INSERT INTO public.legal_case_activities(case_id,company_id,activity_type,activity_title,activity_description,old_values,new_values,created_by)
  VALUES(p_case_id,p_company_id,'judgment_recorded','تسجيل الحكم','تم تسجيل الحكم دون إنشاء حركة نقدية.',
...
BEGIN
  ...
  IF p_next_action_date IS NOT NULL THEN PERFORM public.legal_workflow_create_task_v1(p_company_id,p_case_id,'enforcement:'||v_row.id::text,'متابعة تنفيذ '||v_case.case_number,'متابعة إجراء التنفيذ القادم.',p_next_action_date::timestamptz,'urgent',v_actor); END IF;
  RETURN v_row;
END; $$;
...
BEGIN
  ...
  VALUES(p_case_id,p_company_id,COALESCE(v_case.payment_direction,'receive'),'دفعة مرتبطة بالحكم',p_amount,p_payment_date,NULLIF(BTRIM(COALESCE(p_payment_method,'')),''),'completed',p_invoice_id,p_journal_entry_id,NULLIF(BTRIM(COALESCE(p_notes,'')),''),v_actor,BTRIM(p_reference_number)) RETURNING * INTO v_payment;
  ...
  VALUES(p_case_id,p_company_id,'payment_recorded','تسجيل تحصيل قانوني','تم ربط دفعة فعلية بالقضية.',v_payment.id,to_jsonb(v_payment),v_actor);
  RETURN v_payment;
END; $$;
...
BEGIN
  ...
  VALUES(p_case_id,p_company_id,'case_finally_closed','الإغلاق النهائي للقضية',BTRIM(p_reason),v_actor);
  RETURN v_case;
END; $$;
...
BEGIN
  FOR v_case IN SELECT * FROM public.legal_cases WHERE workflow_stage NOT IN ('closed','cancelled') LOOP
    ...
    IF v_case.hearing_date IS NOT NULL AND v_case.hearing_date <= now()+interval '3 days' THEN
      v_id:=public.legal_workflow_create_task_v1(v_case.company_id,v_case.id,'daily-hearing:'||v_case.id::text||':'||v_case.hearing_date::date::text,'جلسة قريبة: '||v_case.case_number,'مراجعة ملف الجلسة والطلبات قبل الموعد.',v_case.hearing_date-interval '1 day','urgent',v_actor); IF v_id IS NOT NULL THEN v_created:=v_created+1; END IF;
    END IF;
    ...
    IF v_case.workflow_stage IN ('judgment_issued','enforcement','collection') AND COALESCE(v_case.outcome_amount,0)>0 AND COALESCE(v_case.outcome_payment_status,'pending') NOT IN ('paid','received') THEN
      v_id:=public.legal_workflow_create_task_v1(v_case.company_id,v_case.id,'daily-collection:'||v_case.id::text,'متابعة تحصيل الحكم: '||v_case.case_number,'الحكم المالي لم تتم تسويته بالكامل.',now()+interval '1 day','high',v_actor); IF v_id IS NOT NULL THEN v_created:=v_created+1; END IF;
    END IF;
  END LOOP;
