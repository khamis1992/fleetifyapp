...
CREATE OR REPLACE FUNCTION public.generate_amendment_number(
  p_company_id uuid,
  p_contract_id uuid
)
RETURNS text
...
BEGIN
  ...
  IF v_actor_role <> 'service_role' THEN
    IF auth.uid() IS NULL OR public.get_user_company_id() IS DISTINCT FROM p_company_id THEN
      RAISE EXCEPTION 'Not authorized to number amendments for this company' USING ERRCODE = '42501';
    END IF;
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'Manager permission is required to create amendments' USING ERRCODE = '42501';
    END IF;
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Contract not found in the selected company' USING ERRCODE = 'P0001';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended('amendment:' || p_company_id::text || ':' || p_contract_id::text, 0));
  SELECT count(*)
  INTO v_amendment_count
  FROM public.contract_amendments amendment
  WHERE amendment.contract_id = p_contract_id
    AND amendment.company_id = p_company_id;
...
REVOKE ALL ON FUNCTION public.generate_amendment_number(uuid, uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.apply_contract_amendment(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.generate_amendment_number(uuid, uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.apply_contract_amendment(uuid) TO authenticated, service_role;
