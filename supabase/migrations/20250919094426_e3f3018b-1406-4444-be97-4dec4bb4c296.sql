...
CREATE OR REPLACE FUNCTION calculate_basic_confidence(
  ...
  contract_monthly_payment NUMERIC,
...
BEGIN
  ...
  -- Check amount match
  IF payment_amount IS NOT NULL AND contract_monthly_payment IS NOT NULL AND contract_monthly_payment > 0 THEN
    amount_diff := ABS(payment_amount - contract_monthly_payment) / contract_monthly_payment;
