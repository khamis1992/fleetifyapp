-- Admin command to retire legacy zero-amount "paid" invoice placeholders and
-- reissue their billing months through the canonical generator.
--
-- These placeholders (total 0, status 'paid', one zero-balanced posted journal)
-- reserve their canonical month so no collectible invoice can exist for it,
-- while contributing nothing to the ledger. The canonical zero-repair wrapper
-- deliberately refuses them because they are not 'draft', so they accumulate
-- as permanent manual-review findings.
--
-- Safety contract per placeholder (any violation skips the row and reports it):
--   * zero total/subtotal/tax/discount/paid/balance
--   * no payments, no payment allocations, no approval or OCR history
--   * exactly one referenced journal, posted, with zero totals and zero lines
--   * an active contract that passes canonical generation for that month
--
-- The zero journal is marked reversed (audit retained), the placeholder is
-- cancelled, and the month is regenerated through
-- generate_invoice_for_contract_month, which owns numbering, items, schedule
-- linkage, and the balanced posted journal postcondition.
-- p_dry_run = true performs validation only and reports what would change.
...
BEGIN
  ...
  LOOP
    ...
    -- Hard safety gates: any financial, approval, or document history keeps
    -- the row manual-only.
      ...
      ELSIF EXISTS (
        ...
        WHERE entry.company_id = p_company_id
          ...
          AND (
            ...
            OR abs(COALESCE(entry.total_debit, 0)) > 0.01
        ...
        v_reason := 'journal_not_single_zero_posted';
      END IF;
    END IF;
    ...
    BEGIN
      -- Keep the zero journal as audit history, closed as reversed.
      UPDATE public.journal_entries entry
      SET status = 'reversed',
          reversed_at = now(),
          workflow_notes = left(COALESCE(NULLIF(BTRIM(entry.workflow_notes), ''), '')
            || E'\nReversed zero placeholder journal during canonical month repair 20260803223000', 2000),
          updated_at = now()
      WHERE entry.company_id = p_company_id
        AND entry.reference_type = 'invoice'
        AND entry.reference_id = v_invoice.id
      ...
      SET status = 'cancelled',
          ...
          notes = concat_ws(
            ...
            'Retired paid zero placeholder; canonical month reissued 20260803223000'
          ),
          updated_at = now()
...
COMMENT ON FUNCTION public.admin_retire_paid_zero_invoice_placeholders(uuid, boolean) IS
  'Reviewed admin command that retires zero-amount paid invoice placeholders with a reversed zero journal and reissues their months through the canonical generator; dry-run by default.';
