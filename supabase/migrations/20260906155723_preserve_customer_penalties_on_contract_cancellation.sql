...
BEGIN
  ...
  -- Rental cancellation preserves customer traffic liabilities and invoices.
  IF COALESCE(p_transfer_open_penalties_to_company, false) THEN
    RAISE EXCEPTION 'تبقى المخالفات على العميل عند إلغاء العقد؛ أعد المحاولة دون تحويلها إلى الشركة'
      USING ERRCODE = '22023';
  END IF;
  ...
  INSERT INTO public.contract_operations_log (
    ...
    jsonb_build_object(
      ...
      'penalty_resolution', CASE WHEN v_open_count > 0 THEN 'customer_retained' ELSE 'none' END,
      'penalty_ids', to_jsonb(v_penalty_ids),
      'transferred_penalty_count', v_transferred_count,
      'transferred_penalty_amount', 0,
      'retained_penalty_count', v_open_count,
      'retained_penalty_amount', round(v_open_amount, 2),
