-- Atomic, idempotent customer payment batches for quick payment workflows.
...
REVOKE ALL ON FUNCTION public.create_customer_payment_batch_v1(
  uuid, uuid, text, uuid, uuid, text, jsonb, text, uuid
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_customer_payment_batch_v1(
  uuid, uuid, text, uuid, uuid, text, jsonb, text, uuid
) TO authenticated, service_role;

COMMENT ON FUNCTION public.create_customer_payment_batch_v1(
  uuid, uuid, text, uuid, uuid, text, jsonb, text, uuid
) IS 'Atomically and idempotently creates up to 100 invoice payments and updates contract payment dates.';
