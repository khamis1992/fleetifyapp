...
-- First, create the materialized view for dashboard stats
...
-- Now create the safe dashboard stats function
...
BEGIN
    ...
    BEGIN
        ...
        IF result IS NULL THEN
            ...
            WITH stats AS (
                SELECT 
                    (SELECT COUNT(*) FROM vehicles WHERE company_id = company_id_param AND is_active = true) as total_vehicles,
                    (SELECT COUNT(*) FROM customers WHERE company_id = company_id_param AND is_active = true) as total_customers,
                    (SELECT COUNT(*) FROM contracts WHERE company_id = company_id_param AND status = 'active') as active_contracts,
                    (SELECT COUNT(*) FROM employees WHERE company_id = company_id_param AND is_active = true) as total_employees,
                    (SELECT COALESCE(SUM(amount), 0) FROM payments 
                     WHERE company_id = company_id_param 
                     AND payment_method = 'received' 
                     AND payment_status = 'completed'
                     AND payment_date >= CURRENT_DATE - INTERVAL '30 days') as monthly_revenue,
                    (SELECT COUNT(*) FROM vehicle_maintenance 
                     WHERE company_id = company_id_param AND status = 'pending') as pending_maintenance,
                    (SELECT COUNT(*) FROM payments 
                     WHERE company_id = company_id_param 
                     AND due_date < CURRENT_DATE 
                     AND payment_status != 'completed') as overdue_payments,
                    (SELECT COUNT(*) FROM contracts 
                     WHERE company_id = company_id_param 
                     AND end_date <= CURRENT_DATE + INTERVAL '30 days' 
                     AND status = 'active') as expiring_contracts
            )
            SELECT jsonb_build_object(
                ...
                'fleetUtilization', CASE 
                    WHEN total_vehicles > 0 
                    THEN ROUND((active_contracts::decimal / total_vehicles) * 100, 2)
                    ELSE 0 
                END,
