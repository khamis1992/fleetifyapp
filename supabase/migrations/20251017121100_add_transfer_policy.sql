-- Add RLS policy to allow transfer_user_to_company function to work
-- The function runs as SECURITY DEFINER, so we need to allow service_role to bypass RLS

-- Drop and recreate the function to run with proper privileges
...
-- Add comment
COMMENT ON FUNCTION transfer_user_to_company IS 'Transfers a user from one company to another with new roles (Bypasses RLS)';
