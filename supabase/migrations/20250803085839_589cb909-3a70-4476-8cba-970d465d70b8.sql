-- Create function to automatically create invoices for payment schedules
...
-- Update the existing create_contract_payment_schedule function to use the new one
CREATE OR REPLACE FUNCTION public.create_contract_payment_schedule(
    contract_id_param uuid, 
    installment_plan text DEFAULT 'monthly'::text, 
    number_of_installments integer DEFAULT NULL::integer
) RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
