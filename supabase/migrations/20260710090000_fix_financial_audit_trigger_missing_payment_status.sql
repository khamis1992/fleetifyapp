-- Fix financial audit trigger so it can run on invoices and journal_entries.
-- The previous version read OLD.payment_status directly inside a shared
-- trigger function. That fails on tables that do not expose payment_status.
