-- Migration: Create Contract Financial Summary Views
...
-- Add comment to view
COMMENT ON VIEW contract_financial_summary IS 'Provides calculated financial summary for each contract based on invoices, with consistency checks';

-- Create index on contract_id for faster lookups (if not exists)
-- Note: Indexes on views are not supported in PostgreSQL, but the underlying table indexes will be used
