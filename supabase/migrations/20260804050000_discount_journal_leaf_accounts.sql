-- Fix create_invoice_discount_journal_entry account selection: it picked
-- header accounts (blocked by the posting guard) and never matched the
-- Arabic leaf receivable (1200 ذمم العملاء). All three account lookups now
-- require posting-eligible leaf accounts (is_header = false, level >= 3).
