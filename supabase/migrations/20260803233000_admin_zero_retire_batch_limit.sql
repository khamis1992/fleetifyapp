-- Add a bounded batch limit to the zero-placeholder retirement command so an
-- apply run fits within statement timeouts; intended to be called repeatedly
-- until the remaining count reaches zero.
...
BEGIN
  ...
  LOOP
    -- Bound only mutating runs; a dry run always scans the full set quickly.
    IF NOT p_dry_run AND v_attempts >= GREATEST(COALESCE(p_limit, 25), 1) THEN
    ...
    BEGIN
      ...
      IF lower(COALESCE(v_journal.status, '')) = 'draft' THEN
        -- Clear the invoice link before removing the draft journal (FK).
        UPDATE public.invoices invoice
        SET journal_entry_id = NULL, updated_at = now()
        WHERE invoice.id = v_invoice.id
          AND invoice.journal_entry_id = v_journal.id;
      ...
      ELSE
        ...
        SET status = 'reversed',
            ...
            workflow_notes = left(COALESCE(NULLIF(BTRIM(entry.workflow_notes), ''), '')
              || E'\nReversed zero placeholder journal during canonical month repair 20260803233000', 2000),
            updated_at = now()
        WHERE entry.id = v_journal.id
      ...
      SET status = 'cancelled',
          ...
          notes = concat_ws(
            ...
            'Retired paid zero placeholder; canonical month reissued 20260803233000'
          ),
          updated_at = now()
      WHERE invoice.id = v_invoice.id
      ...
      -- Billable contracts get their month reissued through the canonical
      -- generator. Inactive (cancelled/expired) contracts only lose the empty
      -- placeholder; their historical months stay unbilled by design.
      IF EXISTS (
        SELECT 1
        FROM public.contracts contract
        WHERE contract.id = v_invoice.contract_id
          AND lower(COALESCE(contract.status, '')) IN ('active', 'under_legal_procedure')
      ) THEN
      ...
      v_report := v_report || jsonb_build_array(jsonb_build_object(
        ...
        'contract_id', v_invoice.contract_id,
        'month', v_month,
        'action', CASE WHEN v_replacement_id IS NULL THEN 'retired_inactive_contract' ELSE 'retired_and_regenerated' END,
  ...
  RETURN jsonb_build_object(
    ...
    'skipped', v_skipped,
    'remaining_after_run', v_remaining,
    'rows', v_report
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.admin_retire_paid_zero_invoice_placeholders(uuid, boolean, integer)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_retire_paid_zero_invoice_placeholders(uuid, boolean, integer)
  TO service_role;

COMMENT ON FUNCTION public.admin_retire_paid_zero_invoice_placeholders(uuid, boolean, integer) IS
  'Reviewed admin command that retires zero-amount paid invoice placeholders in bounded batches and reissues their months through the canonical generator; dry-run by default.';
