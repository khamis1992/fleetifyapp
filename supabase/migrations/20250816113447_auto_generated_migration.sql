-- إضافة قيود فريدة لمنع التضارب في الحسابات والبنوك
...
BEGIN
    ...
    SELECT array_agg(account_code) INTO existing_codes
    FROM chart_of_accounts
    WHERE company_id = company_id_param AND is_active = true
    LIMIT 20; -- أول 20 فقط للعرض
    
    -- الحصول على أرقام الحسابات البنكية الموجودة
    SELECT array_agg(account_number) INTO existing_bank_accounts
    FROM banks
    WHERE company_id = company_id_param AND is_active = true
    LIMIT 10;
