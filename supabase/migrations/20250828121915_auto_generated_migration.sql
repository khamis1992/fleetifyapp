-- Fix auto_create_customer_accounts function to generate shorter account codes
...
BEGIN
    ...
    BEGIN
        SELECT 
            ...
            CASE 
                ...
                ELSE company_name 
            END
        INTO v_customer_type, v_customer_code, v_customer_name
        FROM customers 
        ...
        RAISE LOG 'auto_create_customer_accounts: Customer found - type=%, code=%, name=%', v_customer_type, v_customer_code, v_customer_name;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        -- Generate unique account code using customer code instead of UUID
        -- Ensure it fits within 20 characters limit
        v_account_code := COALESCE(v_customer_code, v_account_prefix || 'UNK');
        
        -- If the account code is too long, truncate it
        IF LENGTH(v_account_code) > 18 THEN
            v_account_code := LEFT(v_account_code, 18);
        END IF;
        
        -- Add a suffix if account code already exists
        v_counter := 1;
        WHILE EXISTS (SELECT 1 FROM chart_of_accounts WHERE company_id = p_company_id AND account_code = v_account_code) LOOP
            v_account_code := LEFT(COALESCE(v_customer_code, v_account_prefix || 'UNK'), 16) || '-' || v_counter::text;
            v_counter := v_counter + 1;
            
            -- Safety check to prevent infinite loop
            IF v_counter > 99 THEN
                RAISE EXCEPTION 'فشل في إنشاء كود حساب فريد';
            END IF;
        END LOOP;
