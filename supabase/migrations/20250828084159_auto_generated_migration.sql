-- Drop and recreate the auto_create_customer_accounts function with fixed variable names
...
DECLARE
    ...
    new_account_code TEXT;  -- Renamed from account_code to avoid conflict
    new_account_name TEXT;  -- Renamed from account_name to avoid conflict
    new_account_id UUID;
...
BEGIN
    ...
    AND (account_name ILIKE '%receivab%' OR account_name ILIKE '%مدين%' OR account_name ILIKE '%ذمم%')
    AND is_active = true
    LIMIT 1;
