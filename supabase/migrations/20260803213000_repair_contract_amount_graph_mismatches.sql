-- Repair contract_amount for active contracts whose stored amount disagrees
-- with their established billing graph (invoices/schedules), which made the
-- canonical generator reject them with contract_billing_graph_requires_manual_review
-- and left customer-facing balances wrong.
--
-- Rule: the new amount equals the active positive invoice graph total (which
-- equals the active schedule total for every contract fixed here), verified
-- per contract against the current stored amount before any write.
-- LTO2024270 is intentionally excluded: its invoice graph (51,000), schedule
-- graph (51,250), and date formula (54,000) disagree three ways and need
-- manual financial review.
--
-- All previous values are preserved in public._backup_contract_amount_20260803
-- and the matching rollback restores them exactly.
