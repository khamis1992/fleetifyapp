-- Fix the enhanced_cascade_delete_account function to ensure contracts are properly handled
...
BEGIN
    ...
    -- 4. Payments (check if account_id column exists)
    BEGIN
        SELECT COUNT(*) INTO temp_count 
    ...
    IF force_delete = true THEN
        ...
        IF transfer_to_account_id IS NOT NULL THEN
            -- Verify target account exists and is valid for entries
            IF NOT EXISTS (SELECT 1 FROM public.chart_of_accounts WHERE id = transfer_to_account_id) THEN
                RETURN jsonb_build_object(
                    'success', false,
                    'error', 'Transfer target account not found'
                );
            END IF;
