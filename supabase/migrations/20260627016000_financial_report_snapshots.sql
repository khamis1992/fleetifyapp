-- Audit-ready standard financial report snapshots.
-- Published reports keep the source fingerprint and computed totals so exported
-- reports can be tied back to the exact ledger state used at generation time.

CREATE TABLE IF NOT EXISTS public.financial_report_snapshots (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  report_type TEXT NOT NULL
    CHECK (report_type IN ('trial_balance', 'income_statement', 'balance_sheet', 'cash_flow', 'consolidated_trial_balance')),
  period_start DATE,
  period_end DATE,
  as_of_date DATE,
...
BEGIN
  ...
  RETURN jsonb_build_object('snapshot_id', p_snapshot_id, 'status', 'approved');
END;
$$;

CREATE OR REPLACE FUNCTION public.void_financial_report_snapshot(
  p_snapshot_id UUID,
  p_reason TEXT
)
RETURNS JSONB
...
BEGIN
  IF p_reason IS NULL OR length(trim(p_reason)) < 10 THEN
    RAISE EXCEPTION 'A detailed void reason is required';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Financial report snapshot cannot be voided';
  END IF;

  RETURN jsonb_build_object('snapshot_id', p_snapshot_id, 'status', 'voided');
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.approve_financial_report_snapshot(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.void_financial_report_snapshot(UUID, TEXT) TO authenticated;
