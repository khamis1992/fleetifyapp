-- Align the contract.recalculate_totals repair gateway with the deliberate
-- principal cap introduced by 20260725170500 (cap_contract_principal...).
--
-- The gateway verified its postcondition against the UNCAPPED canonical paid
-- amount while recalculate_contract_financial_state stores the CAPPED value.
-- For every overpaid contract the repair therefore failed nightly with
-- 'Contract totals failed canonical postcondition verification' and the
-- finding re-opened forever. Contract summaries represent principal settled
-- against the contract; collections above it remain in the payment ledger.
--
-- The patch is applied surgically to the verified gateway text so no other
-- repair branch is re-typed or altered.
