...
BEGIN
  ...
  END IF;
  -- Reject pre-existing out-of-term schedules before the canonical generator
  -- can bootstrap any new months in this transaction.
  IF EXISTS (SELECT 1 FROM public.contract_payment_schedules s
    WHERE s.company_id = p_company_id AND s.contract_id = p_contract_id
      AND date_trunc('month', s.due_date)::date > date_trunc('month', v_contract.end_date)::date
      AND lower(COALESCE(s.status, '')) NOT IN ('cancelled', 'canceled', 'void', 'voided', 'deleted', 'inactive')) THEN
    RAISE EXCEPTION 'توجد أقساط سابقة خارج المدة؛ راجع الجدول قبل التمديد';
  END IF;
  ...
  WHILE v_month <= date_trunc('month', p_end_date)::date LOOP
    v_schedule_id := NULL;
    SELECT s.id INTO v_schedule_id FROM public.contract_payment_schedules s
    WHERE s.company_id = p_company_id AND s.contract_id = p_contract_id
      AND date_trunc('month', s.due_date)::date = v_month
      AND lower(COALESCE(s.status, '')) NOT IN ('cancelled', 'canceled', 'void', 'voided', 'deleted', 'inactive')
    FOR UPDATE;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.contract_payment_schedules s
      WHERE s.id = v_schedule_id AND abs(s.amount - v_contract.monthly_amount) < 0.01
        AND COALESCE(s.paid_amount, 0) = 0) THEN
      RAISE EXCEPTION 'قسط التمديد لا يطابق المبلغ المطلوب للشهر %', v_month;
    END IF;
