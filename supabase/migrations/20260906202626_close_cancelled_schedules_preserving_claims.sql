...
CREATE OR REPLACE FUNCTION contract_finance_private.schedule_closure_snapshot(p_company uuid, p_contract uuid, p_mode text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
DECLARE c public.contracts%ROWTYPE;s record;snap jsonb;targets jsonb:='[]';blockers jsonb:='[]';
 impact jsonb;invoice_facts jsonb;penalty_facts jsonb;pending_count integer;total numeric:=0;item_ok boolean;
BEGIN
 IF p_mode IS NULL OR p_mode NOT IN('no_claim','preserve_claims') THEN RAISE EXCEPTION 'نوع قرار الإقفال غير صالح' USING ERRCODE='22023'; END IF;
 ...
 END IF;
 IF p_mode='no_claim' AND (snap->>'outstanding')::numeric>0.005 THEN
 ...
 END IF;
 IF p_mode='no_claim' AND (impact->>'open_penalty_count')::integer>0 THEN
 ...
 FOR s IN
  SELECT sch.*,i.id AS evidence_invoice_id,i.invoice_number,i.invoice_month,i.invoice_type,i.penalty_id,i.total_amount,
  ...
  LEFT JOIN LATERAL (
   SELECT candidates.* FROM (
    SELECT inv.*,count(*) OVER() AS candidate_count FROM public.invoices inv
    WHERE inv.company_id=p_company AND inv.contract_id=p_contract
     AND ((sch.cancelled_invoice_id IS NOT NULL AND inv.id=sch.cancelled_invoice_id)
      OR (sch.cancelled_invoice_id IS NULL AND inv.invoice_month=sch.due_date AND inv.total_amount=sch.amount
       AND inv.penalty_id IS NULL AND inv.invoice_type IN('sales','service')
       AND NOT public.financial_record_is_active_v1(inv.status)
       AND NOT public.financial_record_is_active_v1(inv.payment_status)))
   ) candidates WHERE candidates.candidate_count=1
  ) i ON true
 ...
 LOOP
  item_ok:=COALESCE(s.financial_hold_reason IN('cancelled_invoice_obligation_review','cancelled_contract_obligation_review')
  ...
  targets:=targets||jsonb_build_array(jsonb_build_object('id',s.id,'due_date',s.due_date,'amount',s.amount,
   'invoice_id',s.evidence_invoice_id,'invoice_number',s.invoice_number,'eligible',item_ok));
 ...
 SELECT COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.id),'[]') INTO penalty_facts FROM public.penalties p WHERE p.company_id=p_company AND p.contract_id=p_contract;
 RETURN jsonb_build_object('closure_mode',p_mode,'version',1,'company_id',p_company,'contract_id',p_contract,
  ...
  'revision',md5(jsonb_build_object('closure_mode',p_mode,'penalty_facts',penalty_facts,'state',snap-'checked_at','end_date',c.end_date,'targets',targets,
   'invoices',invoice_facts,'penalties',impact,'pending',pending_count,'blockers',blockers)::text));
END;$function$
;
CREATE OR REPLACE FUNCTION contract_finance_private.close_schedule_closure_preserving_claims(p_company uuid, p_contract uuid, p_revision text, p_request uuid, p_reason text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
DECLARE preview jsonb;state jsonb;answer jsonb;old_request contract_finance_private.no_claim_closures%ROWTYPE;
 s public.contract_payment_schedules%ROWTYPE;after_s jsonb;closure_id uuid:=gen_random_uuid();
 n integer:=0;amount_closed numeric:=0;marker text;retained_penalties jsonb;
BEGIN
 ...
 IF FOUND THEN
  IF old_request.contract_id<>p_contract OR old_request.actor_id<>auth.uid()
   OR old_request.result->>'closure_mode' IS DISTINCT FROM 'preserve_claims'
 ...
 PERFORM 1 FROM public.penalties WHERE company_id=p_company AND contract_id=p_contract ORDER BY id FOR SHARE NOWAIT;
 preview:=contract_finance_private.schedule_closure_snapshot(p_company,p_contract,'preserve_claims');
 ...
 END IF;
 marker:='contract_schedule_closure:'||closure_id::text;
 FOR s IN SELECT sch.* FROM public.contract_payment_schedules sch
  ...
  ORDER BY sch.due_date,sch.id
 LOOP
  UPDATE public.contract_payment_schedules SET status='cancelled',financial_hold_reason='resolved_cancelled_schedule',
   cancelled_invoice_id=(SELECT (x->>'invoice_id')::uuid FROM jsonb_array_elements(preview->'schedules') x WHERE (x->>'id')::uuid=s.id),
   notes=concat_ws(E'\n',NULLIF(notes,''),'[إقفال أقساط مع إبقاء المطالبات '||closure_id::text||'] '||btrim(p_reason)),updated_at=now()
  ...
  VALUES(marker,'approved_cancelled_schedules_preserving_claims',p_company,'contract_payment_schedules',s.id,to_jsonb(s),after_s,
   jsonb_build_object('closure_id',closure_id,'actor_id',auth.uid(),'reason',btrim(p_reason),
    'schedule_resolution','closed','block_billing_month',true,'decision','cancelled_schedules_preserving_claims','retained_invoice_amount',preview->'outstanding','retained_penalty_amount',preview->'open_penalty_amount','source','contract_closure_ui'));
 ...
 END IF;
 -- This decision leaves all invoices, penalties, payments and delinquency records intact.
 state:=public.reconcile_contract_financial_integrity_internal_v1(p_company,p_contract,false);
 retained_penalties:=public.get_contract_cancellation_impact_v1(p_company,p_contract);
 IF state->>'status'<>'matched' OR (state->'snapshot'->>'outstanding')::numeric<>(preview->>'outstanding')::numeric
  OR (retained_penalties->>'open_penalty_amount')::numeric<>(preview->>'open_penalty_amount')::numeric
 ...
 answer:=jsonb_build_object('closure_mode','preserve_claims','retained_invoice_amount',preview->'outstanding',
  'retained_penalty_amount',preview->'open_penalty_amount','version',1,'company_id',p_company,'contract_id',p_contract,'closure_id',closure_id,
...
END;$function$
;
CREATE OR REPLACE FUNCTION contract_finance_private.close_no_claim_closure(p_company uuid, p_contract uuid, p_revision text, p_request uuid, p_reason text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
 ...
 IF FOUND THEN
  IF old_request.contract_id<>p_contract OR old_request.actor_id<>auth.uid()
   OR COALESCE(old_request.result->>'closure_mode','no_claim')<>'no_claim'
   OR old_request.revision<>p_revision OR old_request.reason<>btrim(p_reason) THEN
   RAISE EXCEPTION 'استُخدم رقم الطلب لقرار مختلف؛ حدّث المعاينة.' USING ERRCODE='22023';
  END IF;
  RETURN old_request.result||jsonb_build_object('replayed',true);
...
END;$function$
;
CREATE OR REPLACE FUNCTION contract_finance_private.no_claim_snapshot(p_company uuid,p_contract uuid)
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT contract_finance_private.schedule_closure_snapshot(p_company,p_contract,'no_claim');
$fn$;
CREATE FUNCTION contract_finance_private.preview_schedule_closure_preserving_claims(p_company uuid,p_contract uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $fn$
BEGIN
 PERFORM contract_finance_private.require_closure_access(p_company);
 RETURN contract_finance_private.schedule_closure_snapshot(p_company,p_contract,'preserve_claims');
EXCEPTION WHEN no_data_found THEN
 RAISE EXCEPTION 'العقد غير موجود في الشركة الحالية' USING ERRCODE='42501';
END;$fn$;
CREATE FUNCTION public.preview_contract_schedule_closure_v1(p_company_id uuid,p_contract_id uuid)
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT contract_finance_private.preview_schedule_closure_preserving_claims(p_company_id,p_contract_id);
$fn$;
CREATE FUNCTION public.close_contract_schedule_closure_v1(p_company_id uuid,p_contract_id uuid,p_revision text,p_request_id uuid,p_reason text)
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT contract_finance_private.close_schedule_closure_preserving_claims(p_company_id,p_contract_id,p_revision,p_request_id,p_reason);
$fn$;
REVOKE ALL ON FUNCTION contract_finance_private.schedule_closure_snapshot(uuid,uuid,text) FROM PUBLIC,anon,authenticated;
REVOKE ALL ON FUNCTION contract_finance_private.preview_schedule_closure_preserving_claims(uuid,uuid),
 contract_finance_private.close_schedule_closure_preserving_claims(uuid,uuid,text,uuid,text),
 public.preview_contract_schedule_closure_v1(uuid,uuid),public.close_contract_schedule_closure_v1(uuid,uuid,text,uuid,text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION contract_finance_private.preview_schedule_closure_preserving_claims(uuid,uuid),
 contract_finance_private.close_schedule_closure_preserving_claims(uuid,uuid,text,uuid,text),
 public.preview_contract_schedule_closure_v1(uuid,uuid),public.close_contract_schedule_closure_v1(uuid,uuid,text,uuid,text) TO authenticated;
