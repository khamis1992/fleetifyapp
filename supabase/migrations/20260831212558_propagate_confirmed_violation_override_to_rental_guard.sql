...
-- The public wrapper already authenticates the employee, tenant and explicit
-- acceptance. Propagate that decision to the row trigger through a transaction-
-- local, request-bound context. The trigger still blocks legally unavailable
-- vehicles and only skips the unpaid-violation branch for this one atomic insert.
...
BEGIN
  ...
  THEN
    -- Consume the permission so a later insert in the same transaction cannot
    -- inherit the accepted override accidentally.
...
BEGIN
  ...
  IF v_vehicle_penalty_count > 0 OR v_customer_penalty_count > 0 THEN
    ...
    PERFORM pg_catalog.set_config('fleetify.confirmed_violation_override', 'on', true);
    PERFORM pg_catalog.set_config('fleetify.confirmed_violation_override_company_id', p_company_id::text, true);
    PERFORM pg_catalog.set_config('fleetify.confirmed_violation_override_customer_id', p_customer_id::text, true);
    PERFORM pg_catalog.set_config('fleetify.confirmed_violation_override_vehicle_id', COALESCE(p_vehicle_id::text, ''), true);
    PERFORM pg_catalog.set_config('fleetify.confirmed_violation_override_idempotency_key', COALESCE(p_idempotency_key, ''), true);
  END IF;
