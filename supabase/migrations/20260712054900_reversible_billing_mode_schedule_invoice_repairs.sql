...
create or replace function public.system_agent_apply_contract_invoice_billing_month_repair_v6(
  p_run_id uuid,
  p_job_id uuid,
  p_finding_id uuid,
  p_command text,
...
revoke all on function public.system_agent_apply_contract_invoice_billing_month_repair_v6(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) from public, anon, authenticated;
grant execute on function public.system_agent_apply_contract_invoice_billing_month_repair_v6(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) to service_role;

comment on function public.system_agent_apply_contract_invoice_billing_month_repair_v6(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) is 'Canonical reversible contract-level billing-mode schedule invoice repair with optimistic checks, closed-period protection, audit state, and compatible rollback metadata.';
