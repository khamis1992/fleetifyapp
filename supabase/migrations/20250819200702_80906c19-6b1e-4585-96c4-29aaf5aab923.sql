-- Drop and recreate the function with the correct parameter order that matches the RPC call
...
BEGIN
    ...
    IF journal_entries_count > 0 AND NOT force_delete THEN
        can_delete := false;
    ...
    IF account_info.is_system AND NOT force_delete THEN
        can_delete := false;
