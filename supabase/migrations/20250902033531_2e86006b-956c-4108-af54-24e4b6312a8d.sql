-- Enhanced unified function for fast contract creation with simplified approach
...
BEGIN
    ...
    -- Get cost center
    SELECT COALESCE(default_cost_center_id, p_cost_center_id)
    ...
    -- Get receivables account quickly
    SELECT am.chart_of_accounts_id
    INTO v_receivables_account_id
    FROM account_mappings am
    JOIN default_account_types dat ON dat.id = am.default_account_type_id
    WHERE am.company_id = p_company_id 
    AND dat.type_code = 'RECEIVABLES'
    AND am.is_active = true
    LIMIT 1;
    
    -- Fallback to first assets account if no mapping
    ...
    -- Get revenue account quickly  
    SELECT am.chart_of_accounts_id
    INTO v_revenue_account_id
    FROM account_mappings am
    JOIN default_account_types dat ON dat.id = am.default_account_type_id
    WHERE am.company_id = p_company_id 
    AND dat.type_code IN ('RENTAL_REVENUE', 'REVENUE')
    AND am.is_active = true
    ORDER BY CASE WHEN dat.type_code = 'RENTAL_REVENUE' THEN 1 ELSE 2 END
    LIMIT 1;
    
    -- Fallback to first revenue account if no mapping
