-- Allow temporary access for debugging and improve RLS policies
-- Update RLS policies to handle authentication state better
...
-- Create more flexible policies that handle different auth states
...
WITH CHECK (
    ...
    AND (
        ...
        OR (
            get_user_company_safe(auth.uid()) IS NOT NULL 
            AND (company_id = get_user_company_safe(auth.uid()) OR NEW.company_id = get_user_company_safe(auth.uid()))
            AND (
                has_role(auth.uid(), 'company_admin'::user_role) 
                OR has_role(auth.uid(), 'manager'::user_role) 
                OR has_role(auth.uid(), 'sales_agent'::user_role)
