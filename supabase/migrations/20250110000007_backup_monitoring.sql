...
-- Create indexes
CREATE INDEX idx_backup_configurations_company_id ON public.backup_configurations(company_id);
CREATE INDEX idx_backup_configurations_active ON public.backup_configurations(company_id, is_active) WHERE is_active = true;
CREATE INDEX idx_backup_execution_log_config_id ON public.backup_execution_log(backup_config_id);
CREATE INDEX idx_backup_execution_log_status ON public.backup_execution_log(status, start_time);
CREATE INDEX idx_system_monitoring_company_id ON public.system_monitoring(company_id);
CREATE INDEX idx_system_monitoring_metric ON public.system_monitoring(metric_name, recorded_at);
CREATE INDEX idx_system_monitoring_status ON public.system_monitoring(status, recorded_at) WHERE status != 'normal';
CREATE INDEX idx_performance_metrics_company_id ON public.performance_metrics(company_id);
CREATE INDEX idx_performance_metrics_operation ON public.performance_metrics(operation_name, recorded_at);
    ...
    BEGIN
        ...
        LOOP
            ...
            IF EXISTS (
                ...
                -- For this example, we'll just count records
                -- Check if table has company_id column, otherwise count all records
                BEGIN
                    EXECUTE format('SELECT COUNT(*) FROM public.%I WHERE company_id = $1', table_name)
                    INTO total_records
                    USING config_record.company_id;
                EXCEPTION WHEN undefined_column THEN
                    -- Table doesn't have company_id column, count all records
        ...
        SET 
            ...
            next_backup_at = CASE 
                ...
                THEN now() + INTERVAL '1 day' -- Simplified - would use actual cron calculation
                ELSE NULL
            END
...
BEGIN
    ...
    IF metric_status IN ('warning', 'critical') THEN
        INSERT INTO public.smart_alerts_log (
            ...
            p_company_id,
            'system_monitoring',
            format('System Alert: %s', p_metric_name),
            format('Metric %s has reached %s level: %s %s', 
                p_metric_name, 
                metric_status, 
                p_metric_value, 
                COALESCE(p_metric_unit, '')
            ),
            CASE metric_status WHEN 'critical' THEN 'critical' ELSE 'high' END,
            jsonb_build_object(
                'metric_id', metric_id,
                'metric_name', p_metric_name,
                'metric_value', p_metric_value,
                'threshold_warning', p_threshold_warning,
                'threshold_critical', p_threshold_critical
            )
        );
    END IF;
...
BEGIN
    ...
    IF p_execution_time_ms > 5000 THEN -- More than 5 seconds
        PERFORM public.record_system_metric(
            ...
            'ms',
            'performance',
            3000, -- Warning at 3 seconds
            10000, -- Critical at 10 seconds
            jsonb_build_object(
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'performance_metrics' AND table_schema = 'public') THEN
...
-- Trigger to create default backup config for new companies
CREATE OR REPLACE FUNCTION public.trigger_create_default_backup()
RETURNS TRIGGER AS $$
BEGIN
    PERFORM public.create_default_backup_config(NEW.id);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

DROP TRIGGER IF EXISTS trigger_create_default_backup ON public.companies;
CREATE TRIGGER trigger_create_default_backup
    AFTER INSERT ON public.companies
    FOR EACH ROW
    EXECUTE FUNCTION public.trigger_create_default_backup();
