-- Fix the security definer view by removing SECURITY DEFINER and adding proper RLS policies
DROP VIEW IF EXISTS public.payroll_financial_analysis;

-- Create a normal view without SECURITY DEFINER
CREATE VIEW public.payroll_financial_analysis AS
SELECT 
    p.id,
    p.company_id,
    p.payroll_number,
...
CREATE POLICY "Users can view payroll analysis in their company" ON public.payroll_financial_analysis
