-- Shared billing validation: accept complete persisted full or partial installment plans without inferring daily proration.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.contract_payment_schedules s
    WHERE s.company_id = v_contract.company_id AND s.contract_id = v_contract.id
      AND s.financial_hold_reason IS NOT NULL
      AND lower(COALESCE(s.status, '')) NOT IN ('cancelled','canceled','void','voided','deleted','inactive')) THEN
    RAISE EXCEPTION 'Schedule has a financial review hold; resolve it before billing' USING ERRCODE = 'P0001';
  END IF;
  ...
  -- Validated persisted installments define billing amounts; dates alone do not prescribe proration.
