-- Candidate picker for the nightly contract-terms scan: active or legal
-- contracts whose stored contract_amount disagrees with the canonical
-- billing-month total, that own a signed document, and have no open or
-- recently applied proposal. Bounded by p_limit so each run stays small.
...
-- Nightly automatic scan at 03:10 Doha time, after the audit orchestrator and
-- before the daily audit agent. Bounded batches keep the run inside the edge
