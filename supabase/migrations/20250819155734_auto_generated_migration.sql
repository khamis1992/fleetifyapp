-- Fix account hierarchy by properly connecting Cash accounts to Assets (with proper uniqueness)
DO $$
DECLARE
    company_rec RECORD;
    assets_account_id UUID;
BEGIN
    -- For each company, fix the hierarchy
    FOR company_rec IN SELECT DISTINCT company_id FROM public.chart_of_accounts WHERE account_code IN ('1102', '1103') LOOP
        -- Get the Assets account for this company
        SELECT id INTO assets_account_id 
        ...
        -- Update Cash accounts to have Assets as parent (only if Assets account exists)
        IF assets_account_id IS NOT NULL THEN
            UPDATE public.chart_of_accounts 
            SET parent_account_id = assets_account_id
...
BEGIN
  ...
  -- Check other tables...
  SELECT COUNT(*) INTO payments_count FROM public.payments WHERE account_id = account_id_param;
  ...
  SELECT COUNT(*) INTO invoices_count FROM public.invoices WHERE account_id = account_id_param;
  ...
  SELECT COUNT(*) INTO customers_count FROM public.customers WHERE account_id = account_id_param;
  ...
  SELECT COUNT(*) INTO vehicles_count FROM public.vehicles WHERE account_id = account_id_param;
  ...
  IF transfer_to_account_id IS NOT NULL THEN
    ...
    UPDATE public.invoices SET account_id = transfer_to_account_id WHERE account_id = account_id_param;
    UPDATE public.customers SET account_id = transfer_to_account_id WHERE account_id = account_id_param;
    UPDATE public.vehicles SET account_id = transfer_to_account_id WHERE account_id = account_id_param;
    UPDATE public.chart_of_accounts SET parent_account_id = transfer_to_account_id WHERE parent_account_id = account_id_param;

    -- Deactivate the account
    UPDATE public.chart_of_accounts SET is_active = false, updated_at = now() WHERE id = account_id_param;
  ...
  IF force_delete THEN
    ...
    UPDATE public.chart_of_accounts SET is_active = false, updated_at = now() WHERE parent_account_id = account_id_param;

    -- Delete all related data that prevents account deletion
  ...
  -- Default case - can only delete if no constraints
