...
BEGIN
    ...
    FOREACH current_type_code IN ARRAY essential_types
    LOOP
        ...
        ELSE
            ...
            IF account_record.id IS NOT NULL THEN
                ...
                WHERE type_code = current_type_code
                LIMIT 1;
                ...
                IF new_mapping_id IS NOT NULL THEN
                    ...
                    result := jsonb_set(result, '{created}', 
                        (result->'created') || jsonb_build_array(current_type_code));
                ELSE
                    result := jsonb_set(result, '{errors}', 
                        (result->'errors') || jsonb_build_array('Default account type not found: ' || current_type_code));
                END IF;
            ELSE
                -- No suitable account found
                result := jsonb_set(result, '{errors}', 
                    (result->'errors') || jsonb_build_array('No suitable account found for: ' || current_type_code));
            END IF;
        END IF;
    END LOOP;
