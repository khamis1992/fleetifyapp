...
DROP TRIGGER IF EXISTS trigger_update_vehicle_status_on_maintenance ON public.vehicle_maintenance;
DROP TRIGGER IF EXISTS trigger_create_vehicle_fixed_asset ON public.vehicles;

-- Create function to automatically update vehicle status based on maintenance
...
-- Create trigger for vehicle status updates
...
-- Create function to automatically create fixed asset when vehicle is added
...
-- Create trigger for vehicle fixed asset creation
CREATE TRIGGER trigger_create_vehicle_fixed_asset
    AFTER INSERT ON public.vehicles
    FOR EACH ROW EXECUTE FUNCTION public.create_vehicle_fixed_asset();

-- Create function to update vehicle depreciation
...
-- Create function to get available vehicles for contracts
...
-- Add fixed_asset_id column to vehicles table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='vehicles' AND column_name='fixed_asset_id') THEN
        ALTER TABLE public.vehicles ADD COLUMN fixed_asset_id uuid REFERENCES public.fixed_assets(id);
    END IF;
END $$;