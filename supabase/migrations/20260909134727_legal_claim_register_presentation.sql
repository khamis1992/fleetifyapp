-- Disclose rent receipts without adding them to the principal twice.
-- Leave absent base claims without a misleading service period.
DO $guard$ BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='legal_memo_calc_private.claim_register(uuid,uuid,date,jsonb)'::regprocedure) IS DISTINCT FROM 'a9c1c3ff2d65c906bbc002c2f7fc3da4' THEN RAISE EXCEPTION 'Claim register changed; review the migration before applying'; END IF;
END $guard$;
