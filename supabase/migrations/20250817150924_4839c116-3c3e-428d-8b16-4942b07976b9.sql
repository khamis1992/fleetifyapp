...
-- 3. إنشاء دالة لحساب مستوى الحساب بناءً على الهيكل الهرمي
CREATE OR REPLACE FUNCTION calculate_account_level(account_id_param uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
UPDATE public.chart_of_accounts 
SET account_level = calculate_account_level(id)
WHERE company_id IS NOT NULL;
...
-- 6. إضافة قيود لمنع تكرار أكواد الحسابات في نفس الشركة
...
BEGIN
    ...
    FROM public.chart_of_accounts coa
    WHERE coa.account_level != calculate_account_level(coa.id);
