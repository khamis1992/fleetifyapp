-- The legacy sync trigger runs after the canonical amount trigger and normally
-- replaces the amount with an inclusive calendar-month estimate. Preserve an
-- explicit positive amount only inside an audited atomic billing command; keep
-- the legacy behavior unchanged for every ordinary contract update.
