-- إصلاح دالة direct_delete_all_accounts لحل مشكلة عمود account_id غير موجود
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            -- فحص المراجع الأخرى مع التحقق من وجود الأعمدة أولاً
            has_other_refs := false;
            
            -- فحص جدول contracts إذا كان يحتوي على account_id
            IF EXISTS (
            ...
            -- فحص جدول payments إذا كان يحتوي على account_id
            IF NOT has_other_refs AND EXISTS (
                SELECT 1 FROM information_schema.columns 
            ...
            -- فحص جدول invoices إذا كان يحتوي على account_id
            IF NOT has_other_refs AND EXISTS (
                SELECT 1 FROM information_schema.columns 
            ...
            -- فحص جدول customers إذا كان يحتوي على account_id
            IF NOT has_other_refs AND EXISTS (
                SELECT 1 FROM information_schema.columns 
            ...
            ELSE
                ...
                -- 3. تنظيف المراجع الأخرى (تحديث إلى NULL) مع فحص وجود الأعمدة
                IF EXISTS (
                    SELECT 1 FROM information_schema.columns 
                    WHERE table_name = 'contracts' 
                    AND column_name = 'account_id' 
