...
comment on function public.system_agent_apply_contract_invoice_billing_month_repair_v9(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) is 'Canonical reversible billing-mode repair that restores preexisting inactive schedules and removes only schedules created inside the invoice transaction.';
commit;
