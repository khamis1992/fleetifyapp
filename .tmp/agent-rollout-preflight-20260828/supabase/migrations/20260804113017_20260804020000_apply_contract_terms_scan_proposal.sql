...
BEGIN
  ...
  LOOP
    IF COALESCE(v_schedule.paid_amount, 0) > 0.01
       ...
       OR v_schedule.invoice_id IS NOT NULL
    THEN
  ...
  LOOP
    ...
    THEN
      ...
      v_paid_review := v_paid_review || jsonb_build_array(jsonb_build_object(
        ...
        'reason', 'has_payment_history_requires_credit_review'
      ));
      CONTINUE;
    END IF;
    ...
    BEGIN
      ...
      v_new_invoice_id := public.generate_invoice_for_contract_month(
        v_contract.id,
        date_trunc('month', COALESCE(v_invoice.invoice_month, v_invoice.invoice_date))::date
      );
