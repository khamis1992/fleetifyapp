    ...
    BEGIN
      ...
      ELSE
        ...
        SET status = 'reversed',
            ...
            workflow_notes = left(COALESCE(NULLIF(BTRIM(entry.workflow_notes), ''), '')
              || E'\nReversed zero placeholder journal during canonical month repair 20260803233100', 2000),
            updated_at = now()
        WHERE entry.id = v_journal.id
      ...
      SET status = 'cancelled',
          ...
          notes = concat_ws(
            ...
            'Retired paid zero placeholder; canonical month reissued 20260803233100'
          ),
          updated_at = now()
