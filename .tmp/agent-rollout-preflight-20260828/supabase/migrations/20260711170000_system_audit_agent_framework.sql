...
BEGIN
  ...
  ELSIF p_command = 'vehicle.sync_status' THEN
    IF lower(COALESCE(v_before ->> 'status', '')) IN ('accident','stolen','police_station','out_of_service')
       OR (p_values ->> 'status') NOT IN ('available','rented','maintenance','out_of_service','reserved') THEN
