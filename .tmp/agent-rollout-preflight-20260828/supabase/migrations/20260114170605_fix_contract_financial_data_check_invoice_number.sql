...
BEGIN
  ...
  LOOP
    ...
    -- 2. إنشاء الفواتير المفقودة (مع التحقق من رقم الفاتورة)
    FOR i IN 1..v_number_of_months LOOP
      v_current_date := DATE_TRUNC('month', v_contract.start_date) + (i || ' months')::INTERVAL;
      v_invoice_number := 'INV-' || v_contract.c_contract_number || '-' || TO_CHAR(v_current_date, 'YYYY-MM');
      
      IF v_current_date <= DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month' THEN
        -- التحقق من عدم وجود فاتورة بنفس الرقم أو نفس الشهر
        IF NOT EXISTS (
          ...
          WHERE (inv.contract_id = v_contract.id AND inv.status != 'cancelled'
             ...
             OR (inv.company_id = v_contract.company_id AND inv.invoice_number = v_invoice_number)
        ) THEN
          INSERT INTO invoices (
            company_id, customer_id, contract_id, invoice_number, invoice_date, due_date,
            total_amount, subtotal, balance_due, paid_amount, payment_status, status, invoice_type, notes, created_at
          ) VALUES (
            ...
            v_monthly_amount, 0, 'unpaid', 
            CASE WHEN v_current_date < CURRENT_DATE THEN 'overdue' ELSE 'draft' END,
            'sales', 'فاتورة إيجار شهرية - ' || TO_CHAR(v_current_date, 'YYYY-MM'), NOW()
          );
    ...
    -- 3. تزامن حالة جدول الدفعات مع الفواتير (بما في ذلك الفواتير بأرقام مختلفة)
    FOR v_schedule IN
