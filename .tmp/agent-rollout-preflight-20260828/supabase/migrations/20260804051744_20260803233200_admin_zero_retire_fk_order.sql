    ...
    BEGIN
      ...
      ELSE
        ...
        SET status = 'reversed',
            ...
            workflow_notes = left(COALESCE(NULLIF(BTRIM(entry.workflow_notes), ''), '')
              || E'\nReversed zero placeholder journal during canonical month repair 20260803233200', 2000),
            updated_at = now()
        WHERE entry.id = v_journal.id
      ...
      SET status = 'cancelled',
          ...
          notes = concat_ws(
            ...
            'Retired paid zero placeholder; canonical month reissued 20260803233200'
          ),
          updated_at = now()
      WHERE invoice.id = v_invoice.id
...
COMMENT ON FUNCTION public.admin_retire_paid_zero_invoice_placeholders(uuid, boolean, integer) IS
  'Reviewed admin command that retires zero-amount paid invoice placeholders in bounded attempt-limited batches and reissues their months through the canonical generator; dry-run by default.';
