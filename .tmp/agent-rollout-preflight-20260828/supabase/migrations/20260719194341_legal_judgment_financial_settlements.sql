...
BEGIN
  ...
  IF v_case.workflow_stage='closed' AND v_settled<COALESCE(v_case.outcome_amount,0)-0.01 THEN
    ...
    VALUES(p_company_id,p_case_id,'closed_with_balance','Ù‚Ø¶ÙŠØ© Ù…ØºÙ„Ù‚Ø© ÙˆÙ„Ù‡Ø§ Ø±ØµÙŠØ¯ Ø­ÙƒÙ… ØºÙŠØ± Ù…Ø³Ø¯Ø¯',
...
BEGIN
  ...
  VALUES(p_case_id,p_company_id,'judgment_payment_linked','Ø±Ø¨Ø· Ø­Ø±ÙƒØ© Ù…Ø§Ù„ÙŠØ© Ø¨Ø§Ù„Ø­ÙƒÙ…',
    'ØªÙ… Ø±Ø¨Ø· Ø­Ø±ÙƒØ© Ù…Ø§Ù„ÙŠØ© ÙØ¹Ù„ÙŠØ© Ø¨Ø§Ù„Ø­ÙƒÙ… Ø¯ÙˆÙ† Ø¥Ù†Ø´Ø§Ø¡ Ù…Ø¨Ù„Øº Ù‚Ø§Ù†ÙˆÙ†ÙŠ Ù…ÙˆØ§Ø²Ù.',
...
BEGIN
  ...
  VALUES(v_row.case_id,p_company_id,'judgment_payment_unlinked','Ø¹ÙƒØ³ Ø±Ø¨Ø· Ø­Ø±ÙƒØ© Ù…Ø§Ù„ÙŠØ©',BTRIM(p_reason),
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      VALUES(p_company_id,v_match.case_id,v_match.payment_id,'over_allocation',100,'ØªØ¹Ø°Ø± Ø§Ù„Ø±Ø¨Ø· Ø§Ù„ØªÙ„Ù‚Ø§Ø¦ÙŠ Ù„Ø­Ø±ÙƒØ© ØªØ­Ù…Ù„ Ø±Ù‚Ù… Ø§Ù„Ù‚Ø¶ÙŠØ©',jsonb_build_object('error',SQLERRM))
      ON CONFLICT DO NOTHING;
    END;
  END LOOP;
  ...
  WITH candidates AS (
    ...
    INSERT INTO public.legal_settlement_review_items(company_id,case_id,payment_id,issue_type,confidence,title,details)
    SELECT p_company_id,case_id,payment_id,'match_suggestion',score,'Ø­Ø±ÙƒØ© Ù…Ø§Ù„ÙŠØ© Ù…Ø­ØªÙ…Ù„Ø© Ù„Ø­ÙƒÙ… Ù‚Ø¶Ø§Ø¦ÙŠ',jsonb_build_object('score',score)
  ...
  SELECT a.company_id,a.case_id,a.payment_id,'missing_journal','Ø­Ø±ÙƒØ© Ù…Ø±ØªØ¨Ø·Ø© Ø¨Ø§Ù„Ø­ÙƒÙ… Ø¯ÙˆÙ† Ù‚ÙŠØ¯ Ù…Ø±Ø­Ù„',jsonb_build_object('allocation_id',a.id)
...
SELECT lp.company_id,lp.case_id,lp.id,'legacy_payment_unlinked','Ø¯ÙØ¹Ø© Ù‚Ø§Ù†ÙˆÙ†ÙŠØ© Ù‚Ø¯ÙŠÙ…Ø© ØºÙŠØ± Ù…Ø±ØªØ¨Ø·Ø© Ø¨Ø­Ø±ÙƒØ© Ù…Ø§Ù„ÙŠØ©',
