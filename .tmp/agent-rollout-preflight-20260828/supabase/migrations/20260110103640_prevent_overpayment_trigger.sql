-- Migration: Prevent Overpayment Trigger
-- Created: 2025-01-10
-- Description: This migration creates a trigger function that prevents recording
--              payments that would cause a contract to be overpaid beyond reasonable limits.
--
-- The trigger validates:
-- 1. Single payment amounts don't exceed 10x monthly amount or QAR 50,000
-- 2. Total payments don't exceed contract amount by more than 10%
-- 3. Warns when payment is significantly different from invoice amount

-- =========================================
-- Function: validate_payment_amount
-- =========================================
-- Validates payment amounts before allowing insert/update
--
-- Raises an exception if:
-- - Payment amount is suspiciously large (> 10x monthly or > QAR 50,000)
-- - Payment would cause contract to be overpaid by > 10%
--
...
BEGIN
    -- Only validate for contract-linked payments
    ...
    -- Skip validation for zero or negative amounts (they'll be caught by other constraints)
    ...
    -- Skip this check for contracts with zero monthly amount (could be one-time payments)
...
'Validates payment amounts before insert/update to prevent overpayments. Raises exception for suspiciously large payments or payments that would overpay a contract by more than 10%.';
...
-- =========================================
-- Helper Function: check_contract_payment_health
-- =========================================
-- Returns health status of a contract's payments
--
-- Usage:
--   SELECT * FROM check_contract_payment_health('contract-uuid');
--
...
'Returns payment health status of a contract. Use to identify contracts with payment issues.';


-- =========================================
-- View: contract_payment_health_dashboard
-- =========================================
-- Dashboard view showing all contracts and their payment health
--
-- Usage:
--   SELECT * FROM contract_payment_health_dashboard;
--
