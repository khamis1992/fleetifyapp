-- Refresh the stopped filing snapshot and return it to the queue atomically.
-- Keeping both writes in one transaction prevents a refreshed job from being
-- left in needs_human when the second client RPC is interrupted.
CREATE OR REPLACE FUNCTION public.restart_taqadi_filing_job_v2(
  p_company_id uuid,
  p_job_id uuid,
  p_payload jsonb
)
...
REVOKE ALL ON FUNCTION public.restart_taqadi_filing_job_v2(uuid, uuid, jsonb)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.restart_taqadi_filing_job_v2(uuid, uuid, jsonb)
  TO authenticated;

COMMENT ON FUNCTION public.restart_taqadi_filing_job_v2(uuid, uuid, jsonb) IS
  'Atomically refreshes a safely stopped Taqadi filing package and queues it from the beginning.';
