    ...
    IF NEW.invoice_id IS NOT NULL THEN
        ...
        IF FOUND THEN
            ...
            IF v_invoice.total_amount > 0 THEN
                v_invoice_balance_due := COALESCE(v_invoice.balance_due, v_invoice.total_amount);
                
                -- Calculate actual balance due from payments if balance_due seems incorrect
                SELECT COALESCE(SUM(amount), 0)
                INTO v_current_total_paid
                FROM payments
                ...
                -- Use calculated balance if it differs significantly from stored balance_due
                -- Allow small rounding differences (0.01)
                IF ABS(v_invoice_balance_due - (v_invoice.total_amount - v_current_total_paid)) > 0.01 THEN
                    v_invoice_balance_due := v_invoice.total_amount - v_current_total_paid;
                END IF;

                -- Check if this is the first payment (no payments exist or balance_due equals total_amount)
                -- Use tolerance for floating point comparison
                IF v_current_total_paid = 0 OR ABS(v_invoice_balance_due - v_invoice.total_amount) < 0.01 THEN
                    -- First payment - allow up to total_amount (with small tolerance for rounding)
                    IF NEW.amount > v_invoice.total_amount + 0.01 THEN
                        RAISE EXCEPTION USING
                            ERRCODE = '23514',
                            MESSAGE = 'Payment amount exceeds total invoice amount',
                            HINT = 'Payment amount (QAR ' || TO_CHAR(NEW.amount, 'FM999,999,999.00') ||
                                   ') exceeds invoice total (QAR ' || TO_CHAR(v_invoice.total_amount, 'FM999,999,999.00') ||
                                   '). Please verify the payment amount is correct.';
                    END IF;
                ELSE
                    -- Subsequent payment - check against calculated balance
        ...
        IF v_duplicate_payment_count > 0 THEN
            RAISE EXCEPTION USING
                ...
                HINT = 'Idempotency key "' || NEW.idempotency_key ||
                       '" was already used for a payment in the last 30 days. This may be a duplicate request or a retry that should not have happened. If this is intentional, please use a different idempotency key.';
        END IF;
    END IF;
    ...
    IF NEW.payment_date > CURRENT_DATE + INTERVAL '30 days' THEN
        RAISE EXCEPTION USING
            ...
            HINT = 'Payment date is ' || NEW.payment_date::text ||
                   ', which is more than 30 days in the future (current date: ' ||
                   CURRENT_DATE::text || '). Please check the payment date.';
    END IF;
            ...
            IF v_invoice.contract_id != NEW.contract_id THEN
                RAISE EXCEPTION USING
                    ...
                    HINT = 'Payment links to both contract (' || NEW.contract_id::text ||
                           ...
                           v_invoice.contract_id::text || '). Please verify the linking.';
            END IF;
        END IF;
    END IF;
