...
BEGIN
    ...
    IF NEW.contract_id IS NOT NULL THEN
        ...
        IF FOUND THEN
            -- Allow payments for all contract statuses except explicitly blocked ones
            -- This allows payments for: active, under_review, draft, under_legal_procedure,
            -- completed, overdue, cancelled, expired, suspended
            -- Rationale: Even completed/cancelled contracts may have outstanding balances
...
'Server-side validation for payments. Checks for:
1. Contract overpayment prevention (110% limit) - NO status restriction
2. Single payment size limits (10x monthly or QAR 50,000 max)
3. Invoice status (must not be cancelled/voided)
4. Invoice balance validation (cannot exceed remaining balance)
5. Idempotency key uniqueness (no duplicates within 30 days)
6. Payment date sanity check (max 30 days in future)
7. Contract-invoice consistency (invoice must belong to linked contract)

Updated 2026-01-25: Removed contract status restriction to allow payments
for all contract statuses (completed, cancelled, expired, etc.) since they
may still have outstanding balances that need to be collected.';;
