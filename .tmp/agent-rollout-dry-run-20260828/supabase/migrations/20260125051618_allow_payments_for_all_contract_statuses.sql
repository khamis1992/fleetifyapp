-- Allow Payments for All Contract Statuses
-- Removes contract status restriction from payment validation
...
BEGIN
    ...
    IF NEW.contract_id IS NOT NULL THEN
        ...
        IF FOUND THEN
            -- NO STATUS RESTRICTION - Allow payments for all contract statuses
            ...
            IF v_contract_amount > 0 THEN
                ...
                IF v_new_total_paid > v_overpayment_threshold THEN
                    RAISE EXCEPTION USING
                        ...
                        HINT = format(
                            'Current total paid: QAR %.2f, New total after this payment: QAR %.2f, Contract amount: QAR %.2f. Maximum allowed: QAR %.2f (110%% of contract amount).',
            ...
            IF v_monthly_amount > 0 THEN
                ...
                IF NEW.amount > v_max_payment_threshold THEN
                    RAISE EXCEPTION USING
                        ...
                        HINT = format(
                            'Payment amount (QAR %.2f) exceeds maximum allowed. Maximum is 10x monthly (QAR %.2f) or QAR 50,000.',
                            NEW.amount,
                            v_monthly_amount
                        );
                END IF;
                ...
                IF v_invoice.balance_due IS NULL OR v_invoice.balance_due = v_invoice.total_amount THEN
                    IF NEW.amount > v_invoice.total_amount THEN
                        RAISE EXCEPTION USING
                            ...
                            HINT = format(
                                'Payment (QAR %.2f) exceeds invoice total (QAR %.2f).',
                                NEW.amount,
                ...
                ELSE
                    IF NEW.amount > v_invoice.balance_due THEN
                        RAISE EXCEPTION USING
                            ...
                            HINT = format(
                                'Payment (QAR %.2f) exceeds remaining balance (QAR %.2f).',
                                NEW.amount,
    ...
    IF NEW.idempotency_key IS NOT NULL THEN
        ...
        IF v_duplicate_payment_count > 0 THEN
            RAISE EXCEPTION USING
                ...
                HINT = format('Idempotency key "%s" was already used.', NEW.idempotency_key);
        END IF;
    END IF;
    ...
    IF NEW.payment_date > CURRENT_DATE + INTERVAL '30 days' THEN
        RAISE EXCEPTION USING
            ...
            HINT = format('Payment date %s is too far in the future.', NEW.payment_date);
    END IF;
...
'Payment validation - Updated 2026-01-25: Removed contract status restriction to allow payments for all contract statuses.';;
