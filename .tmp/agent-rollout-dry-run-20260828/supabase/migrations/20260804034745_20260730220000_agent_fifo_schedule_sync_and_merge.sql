-- Daily Audit Agent - Phase 1 upgrades
-- 1) allocate_contract_receipts_fifo: batch-mode FIFO allocation of unallocated
--    receipt remainders to the oldest open due invoices of each contract.
-- 2) sync_contract_schedule_payment_state: keep contract_payment_schedules
--    status/paid_amount/paid_date in sync with their linked invoices.
-- 3) invoice_balance_drift_report: canonical dry-run/apply drift detection for
--    invoice financial state (allocation-aware, unlike the legacy naive scan).
-- 4) merge_duplicate_invoice_into_survivor: approval-gated duplicate invoice
--    merge (moves allocations/payments, reverses journal, cancels duplicate).

-- ---------------------------------------------------------------------------
-- 1) FIFO allocation of unallocated receipt remainders
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    -- A payment fully covered by a direct invoice link (invoice_id set, no
    -- active allocations) is already counted by the canonical formula.
      ...
      LOOP
        ...
        IF NOT p_dry_run THEN
          INSERT INTO public.payment_allocations (
            ...
            now(), 'fifo', v_order, 'Daily audit agent FIFO allocation of unallocated receipt'
          );
        END IF;
...
-- ---------------------------------------------------------------------------
-- 2) Sync schedule payment state from linked invoices
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Canonical invoice balance drift report (allocation-aware)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) Approval-gated duplicate invoice merge
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Move allocations: void the duplicate rows and re-create them for the survivor.
  INSERT INTO public.payment_allocations (
    company_id, payment_id, allocation_type, target_id, amount,
    allocated_date, allocation_method, allocation_order, notes
  )
  ...
  -- Re-point direct payment links (payments without active allocations).
  ...
  -- Unlink schedules that pointed at the duplicate and point them at the survivor.
  UPDATE public.contract_payment_schedules schedule
  SET invoice_id = p_survivor_id, updated_at = now()
  ...
  -- Cancel the duplicate with a journal reversal (requires no active payments
  -- on the duplicate, which the moves above guarantee).
