-- إصلاح ترتيب الأرقام العربية (الآحاد قبل العشرات)
...
BEGIN
  ...
  IF v_integer_part >= 1000 THEN
    ...
    -- تحويل رقم الآلاف بالترتيب الصحيح (الآحاد قبل العشرات)
    IF v_thousands = 1 THEN
      v_thousands_text := 'ألف';
    ELSIF v_thousands = 2 THEN
      v_thousands_text := 'ألفان';
    ELSIF v_thousands >= 3 AND v_thousands <= 10 THEN
      v_thousands_text := v_ones[v_thousands + 1] || ' آلاف';
    ELSIF v_thousands >= 11 AND v_thousands <= 19 THEN
      v_thousands_text := v_teens[v_thousands - 9] || ' ألفاً';
    ELSIF v_thousands >= 20 AND v_thousands < 100 THEN
      v_tens_digit := v_thousands / 10;
      v_ones_digit := v_thousands % 10;
      -- الترتيب الصحيح: الآحاد قبل العشرات
      IF v_ones_digit > 0 THEN
        v_thousands_text := v_ones[v_ones_digit + 1] || ' و' || v_tens[v_tens_digit + 1];
      ELSE
        v_thousands_text := v_tens[v_tens_digit + 1];
      END IF;
      v_thousands_text := v_thousands_text || ' ألفاً';
    ELSIF v_thousands >= 100 THEN
      v_hundreds_digit := v_thousands / 100;
      v_thousands_text := v_hundreds[v_hundreds_digit + 1];
      v_thousands := v_thousands % 100;
      IF v_thousands > 0 THEN
        v_thousands_text := v_thousands_text || ' و';
        IF v_thousands >= 20 THEN
          v_tens_digit := v_thousands / 10;
          v_ones_digit := v_thousands % 10;
          IF v_ones_digit > 0 THEN
            v_thousands_text := v_thousands_text || v_ones[v_ones_digit + 1] || ' و' || v_tens[v_tens_digit + 1];
          ELSE
            v_thousands_text := v_thousands_text || v_tens[v_tens_digit + 1];
          END IF;
        ELSIF v_thousands >= 10 THEN
          v_thousands_text := v_thousands_text || v_teens[v_thousands - 9];
        ELSE
          v_thousands_text := v_thousands_text || v_ones[v_thousands + 1];
        END IF;
      END IF;
      v_thousands_text := v_thousands_text || ' ألفاً';
    END IF;
    
    v_result := v_thousands_text;
  ...
  -- معالجة العشرات والآحاد (الآحاد قبل العشرات)
  IF v_integer_part >= 20 THEN
    v_tens_digit := v_integer_part / 10;
    v_ones_digit := v_integer_part % 10;
    IF v_ones_digit > 0 THEN
      v_result := v_result || v_ones[v_ones_digit + 1] || ' و' || v_tens[v_tens_digit + 1];
    ELSE
