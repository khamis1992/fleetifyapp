-- Remove the 'paid' status block - only block cancelled/voided
...
BEGIN
    ...
    IF NEW.invoice_id IS NOT NULL THEN
        ...
        IF FOUND THEN
            ...
            -- Calculate ACTUAL remaining from payments table
            SELECT COALESCE(SUM(amount), 0)
            ...
            -- Only block if payment significantly exceeds remaining (5% buffer)
            IF v_actual_remaining > 0 AND NEW.amount > (v_actual_remaining * 1.05) THEN
                RAISE EXCEPTION USING
                    ERRCODE = '23514',
                    MESSAGE = 'Payment exceeds remaining balance',
                    HINT = 'Remaining: QAR ' || ROUND(v_actual_remaining, 2);
            END IF;
        END IF;
    END IF;
...
'Payment validation - removed paid status block, only blocks cancelled/voided';;
