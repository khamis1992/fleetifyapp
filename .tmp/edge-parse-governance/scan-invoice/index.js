...
function normalizeName(name) {
  if (!name) return "";
  return name.toLowerCase().replace(/[^\u0600-\u06FF\u0750-\u077F\w\s]/g, " ").replace(/\s+/g, " ").trim();
}
function calculateNameSimilarity(extracted, candidate) {
...
async function performCustomerMatching(supabaseClient, extractedData, rawText, companyId, ocrConfidence) {
  const candidates = [];
  try {
    ...
    for (const customer of customers) {
      const customerName = customer.company_name_ar || customer.company_name || `${customer.first_name_ar || customer.first_name || ""} ${customer.last_name_ar || customer.last_name || ""}`.trim();
      ...
      if (customer.contracts && Array.isArray(customer.contracts)) {
        for (const contract of customer.contracts) {
          const carMatchScore = contract.car_number && rawText.includes(contract.car_number) ? 1 : 0;
          const totalConfidence = ocrConfidence * 0.3 + nameSimilarity * 100 * 0.4 + carMatchScore * 100 * 0.2 + 0.1 * 100;
          const matchReasons = [];
    ...
    const avgNameSimilarity = candidates.length > 0 ? candidates.reduce((sum, c) => sum + calculateNameSimilarity(extractedData.customer_name || "", c.name), 0) / candidates.length : 0;
    return {
      best_match: bestMatch,
      all_matches: candidates.slice(0, 10),
      // Top 10 matches
...
serve(async (req) => {
  ...
  try {
    ...
    const { imageBase64, fileName, ocrEngine = "hybrid", language = "auto" } = await req.json();
    if (!imageBase64) {
      return new Response(
...
- Return confidence score based on text clarity`
    ...
    try {
      ...
      const foundFields = fields.filter((f) => extractedData[f]);
    ...
    const { data: profile, error: profileError } = await supabaseClient.from("profiles").select("company_id,is_active").eq("user_id", user.id).eq("is_active", true).maybeSingle();
    ...
    try {
      const { data: invoice, error: insertError } = await supabaseClient.from("invoice_scans").insert({
    ...
    return new Response(
      JSON.stringify({
        ...
        raw_response: extractedText.substring(0, 1e3)
