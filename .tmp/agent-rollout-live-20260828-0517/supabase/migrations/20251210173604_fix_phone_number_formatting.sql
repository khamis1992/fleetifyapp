-- تحديث دالة trigger_daily_report لإصلاح تنسيق رقم الهاتف
      ...
      IF v_company.recipients IS NOT NULL THEN
        ...
        LOOP
          IF (v_recipient->>'isActive')::boolean = true 
            ...
            -- إذا كان الرقم 8 أرقام (رقم قطري بدون كود البلد)، نضيف 974
            IF length(v_phone) = 8 THEN
              v_phone := '974' || v_phone;
            END IF;
            
            -- إذا كان الرقم يبدأ بـ 0974، نزيل الصفر
            IF v_phone LIKE '0974%' THEN
...
-- إنشاء دالة مساعدة لتنسيق أرقام الهواتف القطرية
CREATE OR REPLACE FUNCTION format_qatar_phone(phone TEXT)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
