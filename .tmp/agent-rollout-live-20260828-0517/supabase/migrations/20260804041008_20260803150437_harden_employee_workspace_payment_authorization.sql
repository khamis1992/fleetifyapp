...
BEGIN
  ...
  IF v_registration_metadata ? 'payment_month'
     ...
     AND COALESCE(v_registration_metadata ->> 'payment_month', '') !~ '^[0-9]{4}-(0[1-2]|1[0-2])$'
  ...
  IF v_actor_role <> 'service_role' THEN
    ...
    -- Preserve explicit finance permissions and the product's existing
    -- finance-capable role compatibility. The narrow fallback below remains
    -- limited to an assigned active employee profile.
  ...
  IF p_invoice_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Invoice not found in the requested company'
        USING ERRCODE = 'P0001';
    END IF;
    IF lower(COALESCE(v_invoice.status, '')) IN ('cancelled', 'canceled', 'void', 'voided', 'deleted', 'inactive')
       OR lower(COALESCE(v_invoice.payment_status, '')) IN ('cancelled', 'canceled', 'void', 'voided', 'deleted', 'inactive')
    THEN
  ...
  -- The fallback is intentionally contract-scoped. It does not add employee
  -- roles to the finance authorization list and cannot authorize an unlinked
  -- or invoice, an inactive contract, a cross-company row, or another
  ...
  -- Resolve an idempotent retry only after the invoice/contract rows have been
  -- locked and the actor has been authorized. This must precede overpayment
  -- checks: the original receipt is already part of the canonical paid amount.
  IF v_idempotency_key IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF lower(COALESCE(v_existing.payment_status, '')) IN (
        ...
        RAISE EXCEPTION 'Idempotency key belongs to a non-retryable payment; submit a new key'
          USING ERRCODE = 'P0001';
      END IF;
      ...
      IF v_existing.customer_id IS DISTINCT FROM v_customer_id
         ...
         OR lower(BTRIM(COALESCE(v_existing.payment_type, ''))) IS DISTINCT FROM
            lower(COALESCE(NULLIF(BTRIM(p_payment_type), ''), v_method))
         OR lower(COALESCE(v_existing.payment_status, '')) IS DISTINCT FROM v_status
         OR lower(COALESCE(v_existing.transaction_type::text, '')) <> 'receipt'
         OR (
           p_bank_id IS NOT NULL
           AND v_existing.bank_id IS DISTINCT FROM p_bank_id
         )
         OR v_existing.account_id IS DISTINCT FROM p_account_id
         OR v_existing.cost_center_id IS DISTINCT FROM p_cost_center_id
         OR upper(COALESCE(NULLIF(BTRIM(v_existing.currency), ''), 'QAR')) IS DISTINCT FROM
            upper(COALESCE(NULLIF(BTRIM(p_currency), ''), 'QAR'))
         OR v_existing.reference_number IS DISTINCT FROM
            NULLIF(BTRIM(COALESCE(p_reference_number, '')), '')
         OR v_existing.agreement_number IS DISTINCT FROM
            NULLIF(BTRIM(COALESCE(p_agreement_number, '')), '')
         OR v_existing.check_number IS DISTINCT FROM
            NULLIF(BTRIM(COALESCE(p_check_number, '')), '')
         OR v_existing.notes IS DISTINCT FROM
         ...
         OR COALESCE(
              ...
              v_existing.monthly_amount IS DISTINCT FROM
                NULLIF(v_registration_metadata ->> 'monthly_amount', '')::numeric
            )
         OR COALESCE(
              abs(
              ...
              v_existing.amount_paid IS DISTINCT FROM
                NULLIF(v_registration_metadata ->> 'amount_paid', '')::numeric
            )
         OR COALESCE(
              abs(
              ...
              v_existing.remaining_amount IS DISTINCT FROM
                NULLIF(v_registration_metadata ->> 'remaining_amount', '')::numeric
            )
         OR v_existing.payment_month IS DISTINCT FROM
         ...
         OR v_existing.days_overdue IS DISTINCT FROM
         ...
         OR COALESCE(
              ...
              v_existing.late_fee_amount IS DISTINCT FROM
                NULLIF(v_registration_metadata ->> 'late_fee_amount', '')::numeric
            )
         OR (
  ...
  -- Resolve an implicit/default bank only for a genuinely new payment. An
  -- idempotent retry must not depend on bank configuration changing later.
...
'Creates an idempotent customer receipt, journal, allocation, contract totals, and bank movement in one transaction; finance-authorized actors retain their existing access while an authenticated active profile is restricted to its own assigned active contract.';

-- Completed receipts call this helper in the same transaction. Mirror the
-- narrow assigned-invoice authorization here so the inner financial write does
-- not reject an already-authorized employee receipt or become a broader bypass.
...
BEGIN
  ...
  IF v_actor_role <> 'service_role' THEN
    v_allowed := public.is_finance_action_authorized(
      ...
      ARRAY['finance.payment.create', 'finance.payments.create', 'finance.payments.write', 'payments.create'],
      ARRAY['super_admin', 'admin', 'company_admin', 'manager', 'accountant', 'sales_agent']
    ) OR public.is_finance_action_authorized(
      v_actor,
      v_payment.company_id,
    ...
    IF NOT v_allowed THEN
      v_employee_workspace_allowed :=
        ...
        AND v_payment.contract_id IS NOT NULL
        AND lower(COALESCE(v_payment.transaction_type::text, '')) = 'receipt'
        AND EXISTS (
          SELECT 1
          FROM public.invoices invoice
          JOIN public.contracts contract
            ON contract.id = invoice.contract_id
           AND contract.company_id = invoice.company_id
          ...
          WHERE invoice.id = v_payment.invoice_id
            ...
            AND (invoice.customer_id IS NULL OR invoice.customer_id = v_payment.customer_id)
            AND contract.customer_id = v_payment.customer_id
