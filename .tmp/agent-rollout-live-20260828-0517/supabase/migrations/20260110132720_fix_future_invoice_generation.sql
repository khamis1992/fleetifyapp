-- ================================================================
-- FIX: Prevent Generation of Future Invoices
-- ================================================================
-- This migration adds validation to prevent creating invoices for future months
-- Issue: System was creating invoices for months 2026-2028
-- Solution: Add date validation in invoice generation functions
-- ================================================================

-- ================================================================
-- Fix 1: Update generate_invoice_for_contract_month
-- ================================================================
-- Add validation to prevent future invoice generation
CREATE OR REPLACE FUNCTION generate_invoice_for_contract_month(
  p_contract_id UUID,
  p_invoice_month DATE -- First day of month (e.g.,2024-01-01)
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_due_date DATE;
BEGIN
  -- NEW: Prevent creating invoices for future months
  IF p_invoice_month > DATE_TRUNC('month', CURRENT_DATE)::DATE THEN
    RAISE NOTICE '⛔ Cannot create invoice for future month: %', p_invoice_month;
    RETURN NULL;
  END IF;
  ...
  -- Check if there's already an invoice for this contract in same month/year
  -- Use due_date to determine month (since invoice_month column doesn't exist)
  ...
  -- Set invoice date and due date to 1st of month
...
-- ================================================================
-- Fix 2: Update backfill_contract_invoices
-- ================================================================
-- Add validation to prevent future invoice generation in backfill
...
BEGIN
  ...
  LOOP
    ...
    -- Start from first day of month AFTER contract start date
    ...
    -- Loop through each month from start date to TODAY (not future months)
    -- CHANGED: Use CURRENT_DATE instead of allowing future months
...
-- ================================================================
-- Fix 3: Update generate_monthly_invoices_for_date
-- ================================================================
-- Add validation to prevent future invoice generation
CREATE OR REPLACE FUNCTION generate_monthly_invoices_for_date(
  p_company_id UUID,
  p_invoice_month DATE -- First day of month
)
RETURNS TABLE (
  contract_id UUID,
  invoice_id UUID,
...
-- Verification Query
-- ================================================================
-- Check for any remaining future invoices
SELECT 
  COUNT(*) as future_invoices_count,
  MIN(invoice_date) as earliest_future_date,
  MAX(invoice_date) as latest_future_date
FROM invoices
