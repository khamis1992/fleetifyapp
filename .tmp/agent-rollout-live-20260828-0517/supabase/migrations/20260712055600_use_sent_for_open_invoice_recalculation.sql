-- Keep canonical invoice recalculation within the invoices_status_check values.
...
GRANT EXECUTE ON FUNCTION public.recalculate_invoice_financial_state(uuid)
  TO service_role;
COMMENT ON FUNCTION public.recalculate_invoice_financial_state(uuid) IS
  'Recalculates invoice paid and balance state from canonical allocations; open non-draft invoices use the allowed sent status.';
