-- إصلاح أسماء العملاء في جدول delinquent_customers
-- المشكلة: الدالة تستخدم first_name و last_name (إنجليزي) فقط
-- الحل: استخدام first_name_ar و last_name_ar (عربي) بالأولوية

-- تحديث دالة update_delinquent_customers لاستخدام الأسماء العربية
...
-- تحديث البيانات الموجودة في جدول delinquent_customers لتصحيح الأسماء
UPDATE delinquent_customers dc
SET customer_name = CASE
    WHEN c.customer_type = 'individual' THEN
        TRIM(COALESCE(c.first_name_ar, c.first_name, '') || ' ' || COALESCE(c.last_name_ar, c.last_name, ''))
    ELSE
        COALESCE(c.company_name_ar, c.company_name, '')
END
FROM customers c
WHERE dc.customer_id = c.id
AND dc.customer_name != CASE
    WHEN c.customer_type = 'individual' THEN
        TRIM(COALESCE(c.first_name_ar, c.first_name, '') || ' ' || COALESCE(c.last_name_ar, c.last_name, ''))
    ELSE
        COALESCE(c.company_name_ar, c.company_name, '')
