...
BEGIN
  ...
  SELECT
    ...
    bool_or(
      ...
      AND document.legal_evidence_state = 'active'
      AND COALESCE(
        document.legal_identity_expires_at,
        document.created_at + interval '24 hours'
      ) > now()
    ),
    bool_or(
      document.legal_identity_match_status IN (
        'mismatch', 'failed', 'expired_unverified'
      )
...
-- Resolve legacy OCR work that exceeded the documented 24-hour window before
-- evaluating whether a replacement is required. Orphan historical documents
-- are retained for audit but cannot block or create work for a nonexistent
-- contract.
...
SET legal_identity_match_status = 'expired_unverified',
    ...
    legal_identity_match_reason = COALESCE(
      NULLIF(document.legal_identity_match_reason, ''),
      'Identity verification expired after 24 hours'
    ),
    legal_identity_checked_at = COALESCE(
      document.legal_identity_checked_at,
      now()
    ),
    updated_at = now()
WHERE document.company_id = '24bc0b21-4e2d-4413-9842-31719a3669f4'::uuid
  AND document.document_type IN ('signed_contract', 'signed_contract_image')
  ...
  AND EXISTS (
    SELECT 1
    FROM public.contracts contract
    WHERE contract.id = document.contract_id
      AND contract.company_id = document.company_id
  )
  AND EXISTS (
    SELECT 1
    FROM public.contract_documents conflict_document
    WHERE conflict_document.company_id = document.company_id
      AND conflict_document.contract_id = document.contract_id
      AND conflict_document.legal_evidence_state = 'quarantined'
      AND conflict_document.ocr_review_reason LIKE
