...
-- Migration: Fix payment schedule generation to use monthly_amount
-- Problem: The function was dividing contract_amount by installment_count
-- Solution: Use monthly_amount from the contract for monthly plans

-- Drop existing function to recreate
DROP FUNCTION IF EXISTS create_payment_schedule_invoices(uuid, text, integer, date);
DROP FUNCTION IF EXISTS create_payment_schedule_invoices(uuid, text, integer);

-- Create improved function that uses monthly_amount for monthly plans
...
BEGIN
    ...
    -- FIX: Use monthly_amount for monthly plans instead of dividing contract_amount
    CASE p_installment_plan
        WHEN 'monthly' THEN
            -- Use the actual monthly_amount from contract
            ...
            ELSE
                -- Fallback to calculated amount if monthly_amount not set
        ...
        WHEN 'quarterly' THEN
            installment_count := COALESCE(p_number_of_installments, 
                GREATEST(1, CEIL(installment_count::NUMERIC / 3)));
            -- For quarterly: 3 months worth
            IF contract_record.monthly_amount IS NOT NULL AND contract_record.monthly_amount > 0 THEN
        ...
        WHEN 'semi_annual' THEN
            installment_count := COALESCE(p_number_of_installments, 
                GREATEST(1, CEIL(installment_count::NUMERIC / 6)));
            -- For semi-annual: 6 months worth
            IF contract_record.monthly_amount IS NOT NULL AND contract_record.monthly_amount > 0 THEN
        ...
        WHEN 'annual' THEN
            installment_count := COALESCE(p_number_of_installments, 
                GREATEST(1, CEIL(installment_count::NUMERIC / 12)));
            -- For annual: 12 months worth
            IF contract_record.monthly_amount IS NOT NULL AND contract_record.monthly_amount > 0 THEN
    ...
    ELSE
        -- Default: first payment is on the start date or first of next month
    ...
    FOR i IN 1..installment_count LOOP
        ...
        invoice_number := 'PS-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD((
            SELECT COALESCE(MAX(
                CASE 
                    WHEN invoice_number ~ '^PS-[0-9]{4}-[0-9]+$' 
                    THEN SUBSTRING(invoice_number FROM 9)::INTEGER 
                    ELSE 0 
                END
            ), 0) + 1 
            FROM public.invoices 
            WHERE company_id = contract_record.company_id 
            AND invoice_number LIKE 'PS-%'
        )::TEXT, 6, '0');
...
CREATE OR REPLACE FUNCTION create_contract_payment_schedule(
...
BEGIN
    -- Call the new function that creates both schedules and invoices
...
COMMENT ON FUNCTION create_payment_schedule_invoices IS 
'Creates payment schedules and invoices for a contract. 
FIXED: Now uses monthly_amount from contract instead of dividing contract_amount.
For monthly plan: uses monthly_amount directly
For quarterly: uses monthly_amount * 3
For semi-annual: uses monthly_amount * 6
For annual: uses monthly_amount * 12';
;
