begin;
create or replace function public.system_agent_apply_contract_invoice_billing_month_repair_v8(
  p_run_id uuid,
  p_job_id uuid,
  p_finding_id uuid,
  p_command text,
...
$$;
revoke all on function public.system_agent_apply_contract_invoice_billing_month_repair_v8(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) from public, anon, authenticated;
grant execute on function public.system_agent_apply_contract_invoice_billing_month_repair_v8(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) to service_role;
comment on function public.system_agent_apply_contract_invoice_billing_month_repair_v8(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) is 'Canonical reversible billing-mode repair that removes only schedules absent before and created by the new invoice inside the same transaction.';
commit;
