begin;
create or replace function public.system_agent_apply_contract_invoice_billing_month_repair_v7(
  p_run_id uuid,
  p_job_id uuid,
  p_finding_id uuid,
  p_command text,
...
declare
  ...
  v_preexisting_month_schedule_ids uuid[] := '{}';
...
begin
  ...
  then
    raise exception 'Schedule and active contract lifecycle do not permit billing-month invoice repair';
  end if;
  ...
  if v_candidate_count > 1 then
    ...
    select coalesce(array_agg(schedule.id), '{}')
    into v_preexisting_month_schedule_ids
    from public.contract_payment_schedules schedule
  ...
  if v_created_invoice_id is not null then
    ...
    where generated_schedule.company_id = p_company_id
      ...
      and not (generated_schedule.id = any(v_preexisting_month_schedule_ids))
      and date_trunc('month', generated_schedule.due_date)::date = v_month
      and abs(coalesce(generated_schedule.amount, 0) - coalesce(v_schedule.amount, 0)) <= 0.01
      and abs(coalesce(generated_schedule.paid_amount, 0)) <= 0.01;
  end if;
...
$$;
revoke all on function public.system_agent_apply_contract_invoice_billing_month_repair_v7(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) from public, anon, authenticated;
grant execute on function public.system_agent_apply_contract_invoice_billing_month_repair_v7(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) to service_role;
comment on function public.system_agent_apply_contract_invoice_billing_month_repair_v7(
  uuid,uuid,uuid,text,uuid,text,jsonb,jsonb,jsonb
) is 'Canonical reversible billing-mode repair that removes only duplicate schedules created inside the same invoice-generation transaction.';
commit;
