...
-- إصلاح دالة validate_payment_amount
...
BEGIN
    ...
    IF v_contract_amount > 0 THEN
        ...
        IF v_new_total_paid > v_overpayment_threshold THEN
            RAISE EXCEPTION 'Payment would cause total paid (QAR %) to exceed contract amount (QAR %) by more than 10 percent. Current total paid: QAR %. Please review existing payments before adding more.',
                v_new_total_paid,
                v_contract_amount,
        ...
        IF v_invoice.total_amount > 0 THEN
            ...
            IF v_invoice_difference > (v_invoice.total_amount * 0.20) THEN
                ...
                v_warning_message := format(
                    'WARNING: Payment amount (QAR %s) differs from invoice amount (QAR %s) by QAR %s. Please verify this is correct.',
                    v_payment_amount,
