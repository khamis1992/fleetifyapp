...
foreach ($File in $Files) {
    ...
    # Handle files with timestamp and UUID pattern (format: timestamp-uuid.sql)
    if ($FileName -match '^(\d{14})-([0-9a-f\-]+)\.sql$') {
        $Timestamp = $matches[1]
    ...
    # Handle files with timestamp and UUID pattern (format: timestamp_uuid.sql)
